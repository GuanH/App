using System;
using System.Collections.Generic;
using System.Text;
using System.IO;
namespace App.VocTypes
{   
    public class Voc
    {        
        public static short vocclick = 0;
        public static short typesclick = 0;
        public static short types = 1;               
        public static string[] name  = new string[1] { "學測7000" };
        public static short[] size   =new short[1] { 5734 };
        public static string[] word = new string[] {
"abandon","v. 遺棄;中止",
"abbey","n.修道院",
"abbreviation","n.縮略語",
"abide","v.遵守",
"ability","n.能力;才能",
"able","adj.能;有能力的",
"abnormal","adj.反常的",
"aboard","prep./adv.在(船、飛機、車)上",
"abolish","v.廢除",//18
"abolition","n.廢除",
"aboriginal","adj. adj.原住民的",
"aborigine","n. 土著居民",
"abortion","n.墮胎",
"about","prep./adv.關於",
"above","prep./adv./adj. 在...上面;優於,勝過",
"abroad","adv.在(到)國外",
"abrupt","adj.突然的;魯莽的",
"absence","n.缺席;缺少",
"absent","adj.缺席的",
"absolute","adj.完全的;絕對的",
"absorb","v.吸收;理解",
"absorbing","adj.極有趣的",
"abstract","adj.抽象的",
"abstraction","n.心不在焉/抽象概念",
"absurd","adj.荒唐的",
"abundance","n.豐富",
"academic","adj.學院的;學術的",
"acceleration","n.加速",
"accept","v./n.接受",
"acceptable","adj.可以接受的",
"acceptance","n.接受",
"accessible","adj.易接近的",
"accessory","n.附件",
"accident","n.事故",
"accidental","adj.偶然的;意外的",
"acclaim","v./n.歡呼;歡呼聲",
"accommodation","n 住所",
"accompany","v.陪同",
"accomplish","v.完成",
"accomplished","adj.有才藝的",
"accomplishment","n. [C]才藝;[U]完成",
"accord","v.符合",
"accordingly","adv. 因此",
"account","n.帳戶;描述",
"account","v.認為;解釋",
"accountable","adj. 負有責任的",
"accountant","n.會計師",
"accounting","n.會計學",
"accumulate","v.積累",
"accumulation","n.累積",
"accuracy","n.正確(性)",
"accurate","adj.準確的",
"accusation","n.控訴",
"accuse","v.把...歸咎(於);指控",
"accustom","v.使習慣",
"ace","n.么點牌;能手",
"ache","n./v.疼痛",
"achieve(ment)","v.(n)完成;達到",
"acoustic","adj.聽覺的",
"acquaintance","n.熟人",
"acquire","v.取得",
"acquisition","n.取得;獲得物",
"acre","n.英畝",
"acreage","n.英畝數",
"across","prep./adv.橫越",
"act","n./v.行動;表演",
"action","n.行動;作用",
"active","adj.活躍的;積極的",
"activity","n.活動;活動力",
"actor","n.男演員",
"actress","n.女演員",
"actual","adj.實際的",
"acute","adj.敏銳的",
"ad","n.廣告",
"adapt","v.使適應",
"adaptation","n.適應;改編(本)",
"add","v.增加",
"addict","n.對~有癮的人",
"addiction","n.耽溺",
"addition","n.增加;附加",
"additional","adj.附加的",
"address","n./v.地址;致詞",
"adequately","adv.充分地",
"adjoin","v.毗連",
"adjust(ment)","v.(n.)調整",
"administrative","adj.行政的",
"administrator","n.行政官員",
"admirable","adj.令人欽佩的",
"admiral","n.海軍將官",
"admiration","n.欽佩",
"admire","v.欽佩;欣賞",
"admission","n.加入許可;承認",
"admit","v.承認",
"adolescence","n.青春期",
"adolescent","adj./n.青春期的;青少年",
"adopt","v.採取;收養",
"adult","n.成年人",
"advance","n./v.推進;將...提前",
"advanced","adj.先進的;高等的",
"advancement","n.前進;擢升",
"advantage","n.有利條件;優勢",
"adventure","n.冒險",
"adverse","adj.反對的",
"advert","v.提及",
"advertise(ment)","v.(n)為...做廣告",
"advertising","n.廣告業",
"advice","n.勸告;消息",
"advise","v.勸告;忠告;通知",
"adviser/advisor","n.顧問",
"advisory","adj.提供忠告的",
"advocate","n./v.提倡者;擁護",
"aerospace","n.航空太空(包括大氣層內外)",
"aesthetic","adj.美感的",
"affair","n.事情;事件",
"affect","v.影響;使感動",
"affection","n.愛情",
"affiliated","adj.相關聯的",
"afford","v.類得起;承擔得起",
"afraid","adj.害怕的",
"after","prep./adv./adj.在...之後;以後的",
"aftermath","n.餘波",
"afternoon","n.下午",
"afterward(s)","adv.以後",
"again","adv.再一次",
"against","prep.反對;對著",
"age","n./v.年齡;使變老",
"agency","n.代理機構;仲介",
"agenda","n.議程",
"agent","n.代理人;仲介人",
"aggregate","n.總數",
"aggression","n.侵略",
"aggressive","adj.好鬥的;積極的",
"aging","adj.變老的",
"ago","adv. 在~以前",
"agony","n.痛苦",
"agree(ment)","v.(n)同意",
"agriculture","n.農業",
"ahead","adv.在·前面;領先",
"aid","n./v.幫助",
"aide","n.助理",
"AIDS","n.愛滋病",
"aim","n./v.目標;瞄準",
"air","n.空氣;大氣;天空",
"aircraft","n.航空器;飛機",
"airforce","n.空軍",
"airline","n.航線;航空公司",
"airmail","n.航空郵件",
"airplane","n.飛機",
"airport","n.機場;航空站",
"air-tight","adj.密封的",
"airway","n.航空公司;航線",
"aisle","n.通道",
"akin","adj.類似的",
"alarm","n./v.警報;驚恐",
"alarming","adj.令人憂心的",//no
"alas","interj.哎呀(表悲傷或恐懼)",
"albeit","adv.儘管",
"album","n.一套唱片;相簿",
"alcohol","n.酒精",
"alcoholic","n./adj.酒精飲料;含酒精的",
"alert","adj./v./n.警覺的;警戒",
"algebra","n.代數",
"alien","adj.外國的",
"alienate","v.使不和",
"alienation","n.離間;疏遠",
"alike","adj./adv. 相同的",
"alive","adj.活著的;有活力的",
"all","adj./adv./pron.全部的;全體",
"allegation","n.宣稱",
"allege","v.聲稱",
"allergy","n.過敏性反應",
"alley","n.小巷子;小徑",
"alliance","n.聯盟",
"allied","adj.同盟的",
"allocate","v.撥給",
"allocation","n.配給",
"allot","v. 分配",
"allow","v. 允許",
"almost","adv.幾乎",
"alone","adj./adv.單獨的",
"along","prep./adv.沿著;一起",
"alongside","prep./adv.靠著...旁邊",
"aloud","adv.大聲地",
"alphabet","n.字母系統",
"already","adv.已經;先前",
"also","adv.同樣地",
"alteration","adj.更動",
"alternate","adj./v. 輪流的",
"alternative","adj.替代的;二擇一的",
"alternatively","adv.二擇一地",
"although","conj.雖然;然而",
"altitude","n.高度",
"altogether","adv.完全;總而言之",
"aluminum","n.鋁",
"always","adv.總是;永遠",
"amateur","n./adj.業餘從事者;業餘的",
"amaze(ment)","v.(n)使驚愕",
"amazing","adj.令人驚訝的",
"ambassador","n.大使;使節",
"ambiguity","n.意義含糊",
"ambiguous","adj.含糊的",
"ambition","n.抱負;志向",
"ambitious","adj.野心勃勃的",
"ambulance","n.救護車",
"ambush","v.埋伏突擊",
"amend","v.修改",
"amid","prep.在...當中",
"ammunition","n.彈藥",
"amnesty","n.大赦",
"among","prep.在...之中",
"amount","n./v.共計;總數",
"amuse(ment)","v.(n.)給~提供娛樂",
"analects","n.語錄",
"analogy","n.相似",
"analysis","n.分析",
"analyst","n.分析家",
"analyze","分析",
"ancestor","n.祖先",
"ancestral","adj.祖先的",
"anchor","n.",
"ancient","adj.古代的",
"anecdote","n.軼事",
"angel","n.天使",
"anger","n.生氣",
"angle","n 角度;立場",
"angler","n.釣魚者",
"angry","adj.生氣的",
"angular","adj.有尖角的",
"animal","n.動物",
"ankle","n.足踝",
"anniversary","n.週年紀念",
"announce(ment)","v.(n.) 宣布",
"annual","adj.一年一度的",
"anonymous","adj.不具名的",
"another","adj./pron.另一個",
"answer","n./v.答案",
"ant","n.螞蟻",
"antenna","n.觸鬚;天線",
"anthem","n.聖歌",
"antibiotic","n.抗生素",
"antibody","n.抗體",
"anticipation","n.期盼",
"antique","adj.年代久遠的",
"anxiety","n.焦慮",
"anxious","adj.焦慮的",
"any","adj./adv./pron.任何的;若干",
"anyhow","adv.無論如何",
"anymore","adv.再也(不)",
"anyway","adv.無論如何",
"apart","adj./adv.分開的",
"apartment","n.公寓大樓",
"ape","n.大猩猩",
"apologize","v.道歉",
"apology","n.道歉",
"appal","v.(=appall)使驚駭",
"appalling","adj.令人恐懼的",
"apparent","adj.明顯的",
"appeal","n./v.吸引;呼籲",
"appealing","adj.迷人的",
"appear","v.出現;顯露",
"appearance","n.外貌;顯露",
"appetite","n.胃口;食慾",
"appetizer","n.(主菜前用的) 開胃食物",
"applaud","v.鼓掌",
"applause","n.掌聲",
"appliance","n.器具;裝備",
"applicable","adj.適用的",
"applicant","n.申請人",
"application","n.應用;申請書",
"apply","v.應用;申請",
"appoint","v.任命;指派",
"appointment","n.約定;指派",
"appraisal","n.評價",
"appreciate","v.欣賞;感激",
"appreciation","n.欣賞;感謝",
"apprehension","n.憂慮",
"apprentice","n.學徒",
"approach","n./v.接近;著手處理",
"appropriate","adj.適當的",
"approval","n.同意;認可",
"approve","v.同意;認可",
"April","n.四月",
"apron","n.圍裙;工作裙",
"apt","adj.有...傾向的",
"aptitude","n.資質",
"aquarium","n.水族館",
"arch","n.拱門;拱形",
"archaeology","n.考古學",
"architect","n.建築師;設計師",
"architecture","n.建築學;建築風格",
"archive","n.檔案",
"area","n.區域;範圍",
"arena","n.活動場所(供運動、娛樂的)",
"argue(ment)","v.(n.)爭論;主張",
"arise","v.升起;復活",
"arithmetic","adj./n.算術的",
"arm","n.手臂",
"armchair","n.扶手椅",
"armed","adj.武裝的",
"army","n.軍隊;團體",
"around","prep./adv.各處;大約",
"arouse","v.喚起",
"arrange(ment)","v.(n.) 安排",
"array","n.一長列",
"arrest","v./n.逮捕;拘留",
"arrival","n.到達;到達者",
"arrive","v.到達;抵達",
"arrogant","adj.傲慢的",
"arrow","n.箭",
"art","n.藝術;藝術品",
"article","n.文章;冠詞",
"articulate","v.清楚的表達",
"artificial","adj.人造的",
"artillery","n.大砲",
"artist","n.藝術家;藝人",
"artistic","adj.藝術的",
"as","prep./adv./conj 如同;當...時;雖然",
"ascertain","v.查明",
"ascribe","v.把...歸因於",
"ash","n.灰燼;骨灰",
"ashamed","adj.感到羞恥的",
"aside","adv.在旁邊;除...之外",
"ask","v.詢問;要求",
"asleep","adj.睡著的",
"aspect","n.方面;觀點",
"aspiration","n.願望",
"aspirin","n.阿斯匹靈",
"aspiring","adj.有抱負的",
"ass","n.驢;蠢人",
"assassinate","v.行刺",
"assault","n.攻擊",
"assemble","v.集合",
"assembly","n.與會者;議會",
"assertion","n.紙張",
"assertive","adj.武斷的",
"assertively","adv.武斷地",
"asset","n.資產",
"assign(ment)","v.(n.) 指派",
"assist","v.協助",
"assistance","n.協助",
"assistant","n.助理",
"associate","v.關聯",
"associate","n.夥伴",
"association","n.協會;聯想;合作",
"assume","v.假定;假裝",
"assurance","n.保證",
"assure","v.保證",
"astonish","v.使驚訝",
"astronaut","n.太空人",
"astronomy","n.天文學",
"asylum","n.庇護(所)",
"athlete","n.運動員",
"athletic","adj.運動的",
"atmosphere","n.氣壓;氣氛",
"attach(ment)","v.(n.) 裝上;附加",
"attack","v./n.攻擊",
"attainment","n.達到",
"attempt","v./n.嘗試",
"attend","v.參加",
"attendant","n.服務員",
"attention","n.注意力",
"attic","n.閣樓",
"attitude","n.態度",
"attract","v.吸引;引誘",
"attraction","n.吸引力",
"attractive","adj.有吸引力的",
"auction","n.拍賣",
"audience","n.觀眾;聽眾",
"audio","adj.可聽頻率的",
"audio-visual","adj.視聽的",
"auditorium","n.禮堂",
"August","n.八月",
"aunt","n.姑媽;阿姨",
"Australia","prop./n.澳洲",
"author","n.作者",
"authority","n.權力;當局",
"autobiography","n.自傳",
"automatic","adj.自動的",
"automatically","adv.自動地",
"automobile/auto","n.汽車",
"autonomy","n.自治(權)",
"autumn","n.秋季",
"availability","n.有效",
"avenge","v.報...之仇",
"avenue","n.大道",
"average","adj./n./v.平均的",
"aviation","n.航空(學)",
"avoid","v.避免;躲開",
"await","v.等候;期待",
"awake","adj./v.清醒的;喚醒",
"awaken","v.意識到;使覺醒",
"awaking","adj 使人覺醒的",
"award","v./n 授予;獎品",
"away","adv.離開;不在",
"awful","adj.糟透的",
"awkward","adj.笨拙的;棘手的",
"awkwardly","adv.笨拙地",
"ax","n.斧頭",
"baby","n.嬰兒;寶貝",
"baby-sitter","n.臨時保姆",
"bachelor","n.單身漢;學士",
"back","adj./adv./n./v. 後面的;背部;贊助",
"background","n.背景",
"backward(s)","adv.反方向地",
"bacon","n.燻豬肉;培根肉",
"bacteria","n.細菌",
"bad","adj.不好的;有害的",
"badminton","n.羽毛球",
"bag","n.袋子",
"baggage","n.行李;裝備",
"bail","n.保釋(金)",
"bait","n./v.誘餌;引誘",
"bake","v.烘;烤",
"bakery","n.麵包店",
"balance","n./v.平衡",
"balcony","n.陽臺;樓座",
"bald","adj.禿的",
"ball","n.球;球狀體",
"ballad","n.民謠",
"ballet","n.芭蕾舞",
"balloon","n.氣球",
"ballot","n.無記名投票",
"ballroom","n.舞廳",
"bamboo","n.竹子",
"ban","v.禁止",
"banana","n.香蕉",
"band","n.帶子;樂團",
"bandage","n.繃帶",
"bank","(1)n.銀行",
"bank","(2)n./v.堤;堆積",
"banker","n.銀行家",
"bankrupt","adj./v./n.破產的",
"banquet","n.宴會",
"baptism","n.洗禮",
"bar","n.棒;沙洲;酒吧",
"barbarian","n.野蠻人",
"barbecue","n./v.烤肉",
"barber","n.理髮師",
"barbershop","n. #9l$",
"bare","adj.裸的;不加掩飾的",
"barely","adv.勉強地;露骨地",
"bargain","n./v.買賣;便宜貨",
"bargaining","n.議價",
"bark","n./v.吠叫;咆哮",
"barkeeper","n.酒吧主人",
"bam","n.穀倉;牲口棚",
"barrel","n.大桶",
"barrier","n.障礙物;棚欄",
"bartender","n.酒保",
"base","n./v.基礎",
"baseball","n.棒球",
"based","adj.以...為基礎的",
"basement","n.地下室",
"basic","adj.基本的",
"basin","n.臉盆;洗滌槽",
"basket","n.籃子;簍子",
"basketball","n.籃球",
"bass","n.低音部",
"bastard","n.私生子;令人討厭的人",
"bat","n./v.球棒",
"bath","n.澡缸;沐浴",
"bathe","v.洗澡;沐浴",
"bathroom","n.浴室",
"bathtub","n.浴缸",
"batter","v.毀損",
"battery","n.電池",
"battle","n./v.戰役",
"battlefield","n.戰場",
"bay","n.海灣",
"BBC","n 英國廣播公司",
"B.C.","n.西元前",
"beach","n.海灘",
"beam","n.樑;光束",
"bean","n.豆子",
"bear","n.熊;魯莽之人",
"bear","v.承受;忍受",
"beard","n.(下巴上的)鬍鬚",
"beast","n.野獸",
"beat","n./v.敲擊;打敗",
"beautiful","adj.美麗的",
"beautify","v.美化",
"beauty","n.美人",
"because","conj.因為",
"beckon","v.召喚",
"become","v.變成",
"bed","n.床",
"bedroom","n.臥室",
"bee","n.蜜蜂",
"beef","n.牛肉",
"beer","n.啤酒",
"before","prep./adv./conj. 在...之前",
"beg","v.乞求;乞討",
"beggar","n.乞丐",
"begin","v.開始",
"beginner","n.初學者",
"behalf","n.為了...的利益;代表",
"behave","v.表現;行為舉止",
"behavior","n.行為",
"behind","prep./adv.在...後面;落後",
"behold","v.見到",
"Beijing","n./prop.北京",
"being","n.存在;生物",
"belief","n.信念;信條",
"believe","v.相信",
"bell","n.鐘(聲);鈴(聲)",
"belly","n.腹部;胃",
"belong","v.歸屬於",
"belongings","n.所有物",
"beloved","n.所愛的人",
"below","prep./adv.在...下方",
"belt","n.腰帶;帶狀物",
"bench","n.長板凳",
"bend","n./v.彎曲",
"beneath","prep.在...下面",
"beneficial","adj.有益的",
"benefit","n./v.利益;有益於",
"berry","n.果",
"beside","prep.在旁邊",
"besides","prep./adv.除此之外",
"besiege","v.包圍",
"best","adj./adv.最好的",
"bestseller","n.暢銷品",
"bet","n./v.打賭",
"better","adj./adv.更好的",
"between","prep./adv.介於...之間",
"beverage","n.飲料",
"beware","v.當心",
"bewilder","v.使混亂",
"beyond","prep./adv.在遠處",
"bias","n.成見",
"Biblical","adj.聖經的",
"bicycle/bike","n.腳踏車",
"bid","v.出價;吩咐",
"big","adj.大的",
"bikini","n.三點式泳裝",
"bill","n.帳單;鈔票",
"billion","n.十億;大量",
"billionaire","n.億萬富翁",
"bind","v.捆;綁",
"bingo","n.賓果遊戲",
"binomial","adj.〔數〕二項式",
"biochemistry","n.生物化學",
"biography","n.傳記",
"biological","adj.生物(學)的",
"biology","n.生物學",
"bird","n.鳥",
"birth","n.出生;分娩",
"biscuit","n.小麵包;餅乾",
"bishop","n.主教",
"bite","n./v.一口之量;咬",
"bitter","adj.有苦味的;痛苦的",
"bitterly","adv.酷寒的",
"bitterness","n.苦;悲傷",
"biweekly","adj./adv. 兩週一次的(地)",
"bizarre","adj.古怪的",
"black","adj./n.黑的",
"blackboard","n.黑板",
"blame","n./v.責備;歸咎於...",
"blameworthy","adj.應受責備的",
"bland","adj.溫和的",
"blank","adj./n.空白的",
"blanket","n.毛毯",
"blast","n.一陣疾風",
"blaze","n.烈火;光輝",
"bleak","adj.陰冷(鬱)的",
"bleed","v.流血",
"blend","v.使混合",
"bless","v./n.為...祝福",
"blessing","n.賜福;同意",
"blind","adj./v.瞎的;使看不見",
"blink","v.眨眼;漠視",
"bloc","n.政治集團",
"block","n./v.塊;障礙物;堵住",
"blonde","n.金髮白種人",
"blood","n.血液",
"bloody","adj.血腥的",
"bloom","n./v.花;開花",
"blossom","n.花(叢)",
"blow","n./v.吹;吹氣;打擊",
"blue","adj./n.藍色的",
"blueprint","n.藍圖",
"blues","n.勃魯斯音樂;沮喪",
"bluff","n.虛張聲勢",
"blunder","n.愚蠢的錯誤",
"blur","v.使模糊",
"blush","v./n.臉紅",
"board","n.木板;牌子",
"board","v.登上(船或車等)",
"boast","v.自吹自擂;誇耀",
"boat","n./v.小船;划船",
"bodily","adj.身體的",
"body","n.身體;主要部分",
"bodyguard","n.侍衛",
"boil","v./n.沸騰",
"bold","adj.英勇的;大膽的",
"boldly","adv.大膽地",
"bolt","n.螺栓;閃電",
"bomb","n./v.炸彈;轟炸",
"bond","n.聯結;契約;債券",
"bone","n.骨頭;骨質",
"bonus","n.紅利",
"book","n.書籍",
"book","v.預訂;登記",
"bookcase","n.書架;書櫥",
"bookstore","n.書店",
"boom","n.隆隆聲;景氣",
"boost","v.提升",
"boot","n.(長筒)靴",
"booth","n.售貨棚",
"border","n./v.邊界;比鄰",
"bore","n./v.使厭煩",
"bored","adj.感到厭煩的",
"boring","adj.令人厭煩的",
"born","adj.出生的",
"borrow","v.借入",
"boss","n.老闆",
"botany","n.植物學",
"both","adj./adv./pron.兩者皆",
"bother","v./n.打擾;煩惱",
"bottle","n./v.瓶子;把...裝瓶",
"boulevard","n.林蔭大道",
"bounce","n./v.跳,躍;使反彈",
"bound","adj.很有可能的;準備前往...的",
"bow","(1) n.弓;虹;蝴蝶結",
"bow","(2) v.拉彎;彎曲;鞠躬",
"bowl","n.碗",
"bowling","n.保齡球",
"box","n./v.箱,盒;把...裝箱",
"boxing","n.拳擊",
"boy","n.男孩;少年",
"boycott","v.抵制",
"brace","n.支撐物",
"brain","n.腦袋;智力",
"brake","n./v.煞車",
"branch","n./v.樹枝;分支",
"brand","n.商標;烙印",
"brave","adj.勇敢的",
"bravery","n.勇敢;勇氣",
"breach","v.違犯",
"bread","n.麵包",
"break","n./v.打破;破曉",
"breakdown","n.故障",
"breakfast","n./v.早餐",
"breakthrough","n 突破",
"break-up","n.瓦解",
"breast","n.胸部",
"breath","n.呼吸",
"breathe","v.呼吸",
"breathtaking","adj.使人興奮的",
"breed","n./v.繁衍;滋生",
"breeder","n.繁殖者",
"breeze","n./v.微風;和風",
"brew","v.沖泡;醞釀",
"brick","n.磚",
"bride","n.新娘子",
"bridegroom","n.新郎",
"bridge","n./v.橋樑;艦橋",
"brief","adj./n./v.簡短的",
"briefcase","n.公事包",
"briefing","n.簡報",
"brigade","n.旅;隊",
"bright","adj.明亮的;聰明的",
"brighten","v.照亮",
"brilliant","adj.光輝的;傑出的",
"bring","v.帶來;引起",
"brisk","adj.輕快的",
"Briton","n.英國人",
"broad","adj.寬廣的;概括的",
"broaden","v.加寬",
"broadcast","n./v.廣播;播放",
"brochure","n.小冊子",
"broker","n.經紀人",
"brood","n.一窩",
"brook","n.小河",
"brother","n.兄弟",
"brown","adj./n.棕色的",
"browse","v 噹食;瀏覽",
"brush","n./v.刷子;刷",
"brutal","adj.殘忍的",
"brutality","n.野蠻",
"bubble","n.氣泡;泡影",
"buck/dollar","n.(美)元",
"bucket","n.水桶;桶狀物",
"bud","n./v.芽;發芽",
"Buddha","n.佛陀",
"Buddhism","n.佛教",
"budget","n./v.預算",
"buffalo","n.水牛",
"buffet","n.自助餐;快餐",
"buffet","v.打擊",
"bug","n.蟲子",
"build","v.建造",
"building","n.建築物",
"bulb","n. 球莖(根)",
"bulk","n.大量",
"bull","n.(未閹割的) 公牛",
"bullet","n.子彈",
"bulletin","n.公報;新聞快報",
"bully","n.恃強凌弱者",
"bump","v./n.碰;重擊",
"bun","n.小圓甜麵包;髮髻",
"bunch","n.一束;一群",
"bundle","n.捆;包裹",
"bunk","n.臥鋪",
"bunker","n.媒倉;掩體",
"burden","n./v.負擔;加負擔於...",
"bureau","n.局;處;署",
"bureaucracy","n.官僚",
"burglar","n.竊賊",
"burn","v./n.燃燒;燒傷",
"burst","v./n.爆發;爆炸",
"bury","v.埋",
"bus","n.巴士",
"bush","n.灌木叢",
"business","n.商業;交易",
"businesslike","adj.處事務實的",
"busy","adj./v.忙碌的",
"butcher","n.肉商",
"butter","n./v.奶油;塗奶油於",
"butterfly","n.蝴蝶",
"button","n./v.鈕扣;按鈕",
"buy","v.買",
"cab","n.計程車",
"cabbage","n.甘藍菜",
"cabin","n.客艙",
"cabinet","n.櫥櫃;密室",
"cable","n./v.電纜;有線電視",
"cafe","n.咖啡店;咖啡",
"cafeteria","n.自助餐館",
"cage","n./v.籠子;監牢",
"cake","n.蛋糕",
"calculate","v.計算",
"calculation","n.計算",
"calculator","n.計算機",
"calendar","n.日曆;行事曆",
"calf","n.小牛",
"call","n./v. 呼叫;打電話",
"calm","adj./v. 鎮靜的",
"calorie","n.卡路里",
"camel","n.駱駝",
"camera","n.照相機",
"camp","n./v.營地;露營",
"campaign","n./v.競選活動;戰役",
"campus","n.(大學的) 校園",
"can","n./v.罐頭;金屬容器",
"canal","n.運河",
"cancel","v.取消",
"cancer","n.惡性腫瘤;癌症",
"candidate","n.候選人",
"candle","n.蠟燭",
"candy","n.糖果",
"cane","n.藤條;甘蔗",
"canvas","n 帆布",
"canyon","n.峽谷",
"cap","n./v.帽子;覆蓋",
"capable","adj.有能力的",
"capacity","n.容量;能力",
"cape","n 披肩;海角",
"capital","adj./n.主要的;首都;資本",
"capital(ism)","n.資本主義(制度)",
"capitalist","n.資本家",
"capitol","n.美國州議會大廈",
"captain","n.隊長;船(艦)長",
"caption","n.說明文字",
"captive","adj.被俘虜的",
"captivity","n.囚禁",
"capture","v./n.捕獲;獲物",
"carbon","n. 碳",
"card","n.卡片;紙牌遊戲",
"cardinal","n.紅衣主教",
"care","n./v.關心;照顧",
"career","n.職業",
"carefree","adj.無憂無慮的",
"careful","adj.仔細的",
"careless","adj.粗心大意的",
"caress","v.愛撫",
"cargo","n.貨物",
"carnival","n.嘉年華會",
"carp","n.鯉魚",
"carpenter","n.木匠",
"carpet","n.地毯",
"carriage","n.四輪馬車;運輸",
"carrier","n.運送人;航空母艦",
"carrot","n.胡蘿蔔",
"carry","v.運送;攜帶",
"cart","n./v.貨車;載運",
"cartoon","n.卡通;連環漫畫",
"carve","v.雕刻;開拓",
"case","(1) n.事實;實例",
"case","(2) n./v.容器;把...裝入箱中",
"cash","n./v.現金",
"cashier","n.出納員",
"casino","n.賭場",
"cassette","n.卡式磁帶",
"cast","n./v.投;演員陣容",
"caste","n.種姓(制度)",
"castle","n.城堡",
"casual","adj.偶然的;碰巧的",
"casually","adv.漫不經心的",
"casualty","n.傷亡者",
"catalogue","n.目錄",
"catastrophe","n.災難",
"catch","v./n.抓住;捕獲",
"catcher","n.捕捉者",
"cater","v.承包伙食",
"caterpillar","n.毛蟲",
"cattle","n.牛;牲口",
"causal","adj.具有因果關係的",
"cause","n./v.導致;原因",
"caution","n.謹慎",
"cautious","adj.小心的",
"cautiously","adv.小心地",
"cavalry","n.騎兵(隊)",
"cave","n.洞穴",
"CD","n.(=compact disc)光碟",
"cease","v.停止",
"ceasefire","n.停火協定",
"ceiling","n.天花板",
"celebrate","v.慶祝",
"celebration","n 慶祝",
"celebrity","n.名人(聲)",
"cell","n.小囚房;細胞",
"cellar","n.地窖",
"cemetery","n.墓地",
"census","n.調查",
"cent","n.一分錢",
"center","n./v.使集中;核心",
"centigrade","adj./n.攝氏(的)",
"central","adj.中央的;核心的",
"century","n.世紀",
"ceramic","adj.陶器(藝)的",
"cereal","n.玉蜀黍薄片",
"ceremonious","adj.拘泥形式的",
"ceremony","n.典禮;禮儀",
"certain","adj.確信的",
"certificate","n.證書",
"chain","n./v.鏈條;束縛",
"chair","n.椅子",
"chairwoman","n.女主席",
"chalk","n.白堊;粉筆",
"challenge","n./v.挑戰",
"challenging","adj.富有挑戰性的",
"chamber","n.房間;室",
"champagne","n.香檳",
"champion","n.優勝者;戰士",
"championship","n.冠軍稱號;錦標賽",
"chance","n./v.偶然;機會",
"Chancellor","n.司法官",
"changeable","adj.易變的",
"channel","n./v.海峽;溝渠",
"chaos","n.無秩序狀態",
"chapter","n.章,回",
"character","n.性格;特質",
"characteristic","adj./n.特有的;特性",
"charge","n./v.索價;控告",
"charitable","adj.仁慈的",
"charity","n.慈善團體",
"charm","nJv.魅力;使陶醉",
"charming","adj.有魅力的",
"chart","n./v.圖表",
"chase","v./n.追逐",
"chat","v./n.聊天",
"chatterbox","n、喋喋不休的人",
"cheap","adj.便宜的",
"cheat","v./n.作弊;欺騙",
"check","n./v.支票;檢查",
"checkbook","n.支票簿",
"check-in","n.登記手續",
"check-out","n.檢查;付款退租",
"checkup","n.檢查",
"cheek","n.臉頰",
"cheer","n./v.歡呼;激勵",
"cheerful","adj.興高采烈的",
"cheerleader","n.啦啦隊隊長",
"cheers","interj.乾杯",
"cheese","n.乳酪;乾酪",
"chef","n.主廚",
"chemical","adj./n.化學的",
"chemist","n.化學家",
"chemistry","n.化學;化學作用",
"cherish","v.珍惜",
"cherry","n.櫻桃",
"chess","n.西洋棋",
"chessboard","n.西洋棋棋盤",
"chest","n.胸膛;箱子",
"chew","v./n.咀嚼;深思",
"chick","n.小雞;少婦",
"chicken","n.雞肉;膽小鬼",
"chief","adj./n.主要的;首領",
"child","n.小孩子",
"childbirth","n.分娩",
"childcare","n.照料小孩",
"childhood","n.童年時期",
"childish","adj.孩子氣的",
"childlike","adj.天真無邪的",
"children","n. [pl.]小孩子",
"chill","v./adj.(使)變冷;寒冷的",
"chilly","adj. 冷颼颼的",
"chimney","n.煙囪",
"chimpanzee","n.黑猩猩",
"chin","n.下巴",
"china","n.瓷器",
"chip","n./v.晶片;削",
"chlorine","n.氯(氣)",
"chocolate","n.巧克力",
"choice","n./adj.選擇",
"choir","n.合唱隊",
"choke","v./n.窒息;阻塞",
"cholesterol","n.膽固醇",
"choose","v.選擇",
"chop","v./n.劈;肋排肉",
"chopstick(s)","n.筷子",
"chorus","n.合唱隊",
"Christianity","n.基督教",
"chronic","adj.慢性的",
"chunk","n.厚塊",
"church","n.教堂",
"churchyard","n.教堂的庭院(墓地)",
"cigar","n.雪茄",
"cigarette","n.香煙;煙捲",
"circle","n./v.循環;畫圓圈",
"circulate","v.循環;流通",
"circulation","n.循環;流通;貨幣",
"circumstance","n.情況;詳情",
"circus","n.馬戲團",
"citizen'","n.公民;市民",
"citizenship","n.公民身分(權利)",
"city","n.城市",
"civics","n.公民科",
"civilian","n.平民;文職人員",
"civilization","n.文明",
"civilize","v.使開化",
"claim","n./v.要求;聲稱",
"clarify","v.澄清",
"clash","n./v.衝突;發出鏗鏘聲",
"class","n./v.階級;班級",
"classic","adj./n.經典的;名著",
"classical","adj.極好的;古典的",
"classification","n.分類",
"classmate","n.同班同學",
"classroom","n.教室",
"clause","n.子句;條款",
"claw","n.爪子;鉗;贅",
"clay","n.黏土;泥土",
"clean","adj./adv./v.乾淨的",
"cleaner","n.清潔工(劑)",
"cleaning","n.掃除",
"cleanse","v.使清潔",
"clear","adj./v.清楚的",
"clergyman","n.神職人員;牧師",
"clerk","n.店員",
"clever","adj.伶俐的;機敏的",
"click","v.使發出聲",
"client","n.客戶",
"cliff","n.懸崖",
"climate","n.氣候;風氣",
"climax","n.高潮",
"climb","v.攀爬",
"clinic","n.診所",
"clip","v.修剪;夾住",
"clock","n.時鐘",
"clockwise","adj.順時針方向的",
"clockwork","n.發條裝置",
"close","adj./adv.接近的",
"close","v.關上;結束",
"closet","n.壁櫥;衣櫥",
"closure","n.關閉",
"cloth","n.布料;衣料",
"clothe","v.為...穿衣",
"clothes","n.衣服",
"clothing","n.衣服;覆蓋物",
"cloud","n./v.雲;遮蔽",
"cloudy","adj.多雲的",
"clown","n.小丑",
"club","n./v.俱樂部;募集",
"clue","n.線索",
"clumsy","adj.笨拙的",
"cluster","n.群",
"clutch","n.離合器;緊握",
"coach","n./v.教練員;公車",
"coal","n.煤礦;煤塊",
"coalition","n.聯合",
"coarse","adj.粗糙的",
"coast","n.海岸;沿海地區",
"coastguard","n.海岸巡邏隊",
"coastline","n.海岸線",
"coat","n./v.外套;覆蓋",
"cocaine","n.古柯鹼",
"cock","n.公雞",
"cockpit","n.駕駛艙;鬥雞場",
"cockroach","n.蟑蚣卜;油蟲",
"cocktail","n.雞尾酒",
"coconut","n.椰子",
"code","n./v.密碼;法典",
"coed","adj.男女合校的",
"coeducation","n.男女合校的教育",
"coexist","v.同時共存",
"coffee","n.咖啡",
"coffin","n.棺材",
"coin","n.硬幣",
"coincidence","n.巧合",
"coke","n.可口可樂",
"cola","n.可樂類飲料",
"collaborate","v.合作",
"collapse","v./n.崩塌",
"collar","n.衣領;項圈",
"colleague","n.同事",
"collect","v.收集;收藏",
"collection","n.收藏品;收集",
"collector","n.收藏者",
"college","n.大學;學院",
"collision","n.衝突",
"colonial","adj.殖民地的",
"colony","n.殖民地",
"color","n./v.顏色;彩色",
"color-blind","adj.色盲的",
"colorful","adj.多采多姿的",
"column","n.欄;圓柱",
"columnist","n.專欄作家",
"comb","n./v.梳子;梳頭",
"combination","n.聯合(體)",
"combine","v.結合",
"come","v.來到;甦醒",
"comedian","n.丑角式人物",
"comedy","n.喜劇",
"comet","n.彗星",
"comfortably","adv.舒適地",
"comforting","adj.舒適的",
"comic","adj.滑稽的",
"coming","n.到來",
"command","n./v.命令;控制",
"commander","n.指揮者",
"commemorate","v.為慶祝",
"commend","v.稱讚",
"comment","n./v.評論",
"commentary","n.評述",
"commentator","n.評論家",
"commerce","n 商業;交易",
"commercial","adj./n.商業的",
"commission","n./v.佣金;委託",
"commit","v.犯(罪);把...託交給...",
"commitment","n.承諾",
"committee","n.委員會",
"commodity","n.商品",
"common","adj./n.一般的",
"commonplace","adj.平凡的",
"communal","adj.公共的",
"communicate","v.溝通",
"communication","n.溝通;訊息",
"communicative","adj.愛說話的",
"communist","n./adj./n.共產主義的;共產主義者",
"community","n.社區;共同體",
"commute","v.通勤",
"commuter","n 通勤者",
"compact","adj.緊密的",
"companion","n.夥伴;伴侶",
"companionship","n.交誼",
"company","n.公司;同伴",
"comparative","adj.相比而言的",
"comparatively","adv.比較地",
"compare","v.比較",
"comparison","n.比較;對照",
"compartment","n.分隔間",
"compass","n.指南針;圓規",
"compassionate","adj.有同情心的",
"compatible","adj.相容的",
"compatibly","adv.和諧地",
"compensate","v.賠償",
"compensation","n.補償",
"compete","v.競爭;比賽",
"competence","n.能力",
"competition","n.競賽;競爭",
"competitive","adj.競爭的",
"competitor","n.競爭者",
"compile","v.編纂",
"complain","v.抱怨",
"complaint","n.抱怨",
"complement","n.補足物",
"complete","adj./v.完全的;完成",
"complex","adj./n.複雜的;情結",
"complexion","n.氣色",
"complexity","n.錯綜複雜的事物",
"complicate","v.使複雜化",
"complicated","adj.複雜的",
"complication","n.糾葛",
"compliment","n./v.讚美;祝賀",
"comply","v.依從",
"component","n.零件;成分",
"compose","v.作曲;組成",
"composer","n.作曲家;調停者",
"composite","adj.混合成的",
"composition","n.作文;合成物",
"comprehend","v.理解",
"comprehensible","adj.可以理解的",
"comprehension","n.理解(力)",
"comprehensive","adj.廣泛的",
"compress","v.壓縮",
"compromise","v.妥協(方案)",
"compulsively","adv.強制性地",
"compute","v.估算;推算",
"computer","n.電腦",
"computerize","v.使電腦化",
"comrade","n.同伴",
"conceal","v.隱藏",
"concede","v.讓與",
"conceive","v.認為;構想",
"conceited","adj.自大的",
"conceitedly","adv.自負地",
"concentrate","v.專心",
"concentration","n 專心;集中",
"concern","n./v.關心;涉及",
"concert","n.演奏會;音樂會",
"concession","n.讓步",
"concise","adj.簡潔的",
"conclude","v.結束;斷定",
"conclusion","n.結論;推論",
"concrete","adj./n.具體的;混凝土",
"condense","v.使凝結(濃縮)",
"condition","n./v.情況;決定",
"condom","n.保險套",
"conductor","n.領導者;指揮",
"confederation","n.同盟",
"confer","v.商議;授予",
"conference","n.會議",
"confess","v.坦承",
"confession","n.承認",
"confide","v.吐露",
"confidence","n.信心;自信",
"confident","adj.有自信的",
"confidently","adv.確信地",
"confidential","adj.機密的",
"confine","v.限制;使侷限",
"confirm","v.詮實",
"conflict","n./v.衝突",
"conform","v.遵從",
"conformity","n.遵從",
"confront","v.面臨;對抗",
"confrontation","n.對質;對抗",
"Confucius","n.孔子",
"confuse","v.把...弄糊塗;攪亂",
"confused","adj 困惑的",
"confusion","n.混亂;困惑",
"congratulate","v.恭喜",
"congratulation","n.恭喜",
"congress","n.正式大會;立法機關",
"congressional","adj.國會的",
"conjunction","n.結合",
"connect","v.連結",
"connection","n.連接;關聯",
"conquer","v.戰勝",
"conquest","n.征服",
"conscientious","adj.盡責的",
"conscious","adj.神智清醒的",
"consciously","adv.有意地",
"consciousness","n.意識;察覺",
"consecutive","adj.連續的",
"consensus","n.多數人的意見",
"consent","n./v.同意",
"consequence","n.結果;重要性",
"consequent","adj.因...的結果而起的",
"conservation","n.保護",
"conservative","adj.保守的",
"consider","v.認為;考慮",
"considerable","adj.值得考慮的",
"considerate","adj.設想周到的",
"consideration","n.考慮;動機",
"consist","v.組成;存在於...",
"consistent","adj.一致的",
"consolation","n.慰藉",
"console","v.安慰",
"consolidate","v.鞏固",
"conspiracy","n.陰謀",
"constable","n.警察",
"constant","adj.固定的",
"constitute","v.構成;制定",
"constitution","n.憲法;法規",
"constitutional","adj.憲法的",
"constraint","n.限制",
"construct","v.建造",
"construction","n.建造物;建設",
"constructive","adj.建設性的",
"consult","v.與...商量",
"consultant","n.顧問;諮詢者",
"consultation","n.協商會",
"consume","v.消耗;花費",
"consumer","n.消費者",
"consumption","n.消費",
"contact","n./v.接觸;聯繫",
"contagious","adj.傳染性的",
"contain","v.包含",
"container","n.容器",
"contaminate","v.污染物",
"contemplate","v.深思",
"contempt","v.輕視",
"contemptible","adj.卑劣的",
"contend","v.競爭;鬥爭",
"content","adj./v.滿足的",
"content(s)","n.內容",
"contented","adj.滿足的",
"context","n.上下文",
"continent","n.大陸;大洲",
"continental","adj.洲的;大陸的",
"contingent","adj.因情況而變的",
"continual","adj.不間斷的",
"continuation","n.繼績",
"continue","v.持續",
"continuous","adj.連續的",
"contract","n./v.契約",
"contractor","n.立約人",
"contradict","v.否認",
"contradiction","n.矛盾",
"contrary","adj./n.相反的",
"contrast","n./v.對比",
"contribute","v.捐獻;貢獻",
"contribution","n.貢獻;捐獻",
"control","n./v.控制",
"controller","n.控制者",
"controversial","adj.有爭議的",
"convene","v.召集",
"convenience","n.便利設施;方便",
"convenient","adj.便利的",
"convention","n.慣例;會議",
"conventional","adj.慣例的;協定上的",
"conversation","n.會話;談吐",
"converse","v.談話",
"convert","v.轉換",
"convertible","adj.可轉換的",
"convey","v.運輸;傳遞",
"convict","v./n.證明有罪;囚犯",
"convince","v.說服",
"convincing","adj.令人信服的",
"convoy","n.護送(車隊)",
"cook","n./v.廚師;烹煮",
"cooker","n.炊具",
"cookie/cooky","n.甜餅乾",
"cool","adj./v.涼爽的;使冷卻",
"cooperate","v.合作",
"cooperation","n.合作",
"cooperative","adj.合作的",
"coordination","n.協調",
"cop","n.警察",
"cope","v.競爭;妥善處理",
"copilot","n.飛機副駕駛",
"copy","n./v.副本;拷貝",
"copyright","n.版權",
"cord","n.細繩;粗線",
"corn","n.玉米;穀物",
"comer","n.角落",
"corporation","n.公司",
"correct","adj./v.正確的;更正",
"correlation","n.關聯",
"correspond","v.符合;通信",
"correspondent","n./adj.通訊(記)者;一致的",
"corrupt","adj./v.腐敗的;敗壞",
"corruption","n.道德敗壞",
"cosmetic","n./adj.化妝品;美容的",
"cosmic","adj.宇宙的",
"cosmopolitan","adj.世界性的",
"cosmos","n.宇宙",
"cost","n./v.花費;付出",
"costly","adj.貴重的;代價高的",
"costume","n.服裝;戲服",
"cosy","adj.(=cozy)溫暖舒適的",
"cottage","n.農舍;小屋",
"cotton","n.棉",
"couch","n.長沙發;睡椅",
"cough","v.咳嗽",
"council","n.會議",
"councilman","n.議員",
"counsel","n.忠告",
"counseling","n.勸告",
"counsellor","n.顧問",
"count","n./v.計算",
"countable","adj.可數的",
"countenance","n.表情;支持",
"counter","n.櫃檯;籌碼",
"counter","adj./adv./n./v.相反的;反擊",
"counteract","抵消",
"counterattack","n./v.反擊",
"counterpart","n.互相對應的人(物)",
"countless","adj.無數的",
"country","n.國家;鄉下",
"countryside","n.鄉村",
"county","n.郡;縣",
"couple","n.(一)對",
"coupon","n.優待券",
"courage","n.勇氣",
"courageous","adj.勇敢的",
"course","n.課程;過程",
"court","n.法庭",
"courteous","adj.有禮貌的",
"courtesy","n.有禮的舉止",
"courtyard","n 庭院",
"cousin","n.表(兄弟姊妹)",
"cover","n./v.封面;遮蓋",
"coverage","n.報導;某事所包含的範圍",
"cow","n.母牛",
"coward","adj./n.懦弱的;懦夫",
"cowardice","n.膽小",
"cowboy","n.牛仔;牧牛人",
"coworker","n.同事",
"crab","n.蟹",
"crack","n./v.爆烈聲;裂開",
"crackdown","n.取締",
"cradle","n./v.搖籃;撫育",
"craft","n.工藝;太空船",
"cram","v.填塞",
"crane","n.起重機;鶴",
"crash","n./v.墜毀;撞擊",
"crawl","v.爬行;蠕動",
"crayon","n.蠟筆",
"crazy","adj.瘋狂的",
"cream","n.奶油;乳脂",
"create","v.創造",
"creation","n.創作;萬物",
"creative","adj. 有創造力的",
"creativity","n.創造力",
"creator","n.創作者",
"creature","n.生物",
"credibility","n.可信性",
"credible","adj.可信的",
"credit","n./v.信用;賒帳",
"creditable","adj.值得稱讚的",
"creed","n.信條",
"creep","v.躡手躡腳地走",
"crew","n.(一群)工作人員",
"cricket","n.蟋蟀",
"crime","n.罪行;犯罪活動",
"criminal","adj/n.犯罪的;罪犯",
"cripple","n.肢體殘障者",
"crisis","n.危機",
"critic","n.評論家",
"critical","adj.關鍵性的;批判的",
"criticism","n.評論",
"criticize","v.批評",
"crocodile","n.鱷魚",
"crop","n.作物",
"cross","n./v.交叉;越過",
"crossing","n.交叉口",
"crossroad","n.十字路口",
"crouch","v.蹲伏",
"crow","n.烏鴉;雞啼聲",
"crowd","n./v.人群;擠滿",
"crowded","adj.擁擠的",
"crown","n./v.王冠;為...加冕",
"crude","adj.未加工製造的",
"cruel","adj.殘酷的",
"cruise","n.乘船遊覽",
"crumble","v.(使)碎成細屑",
"crush","v./n.弄碎;壓碎",
"cry","v./n.哭;叫喊",
"crystal","adj./n.水晶的",
"cub","n.幼獸",
"cube","n.立方體",
"cubic","adj.立方的",
"cucumber","n.黃瓜",
"cue","n.反芻的食物",
"culminate","v.達到頂點",
"cult","n.狂熱的崇拜",
"cultivate","v.耕作;培養",
"cultural","adj.文化的;教養的",
"culture","n.文化;教養",
"cumulative","adj.累積的",
"cunning","adj.狡猾的;奸詐的",
"cup","nJv.杯子",
"cupboard","n.碗櫃",
"curb","n.抑制",
"cure","n/v.治療;治癒",
"curfew","n.宵禁",
"curiosity","n.好奇心",
"curious","adj.好奇的",
"currency","n.貨幣;流通",
"current","adj./n.目前的;潮流",
"curriculum","n.課程",
"curry","n.咖哩食品",
"curtain","n./v.簾;給...裝上簾子",
"curve","n./v 曲線;使彎曲",
"cushion","n.墊子",
"custody","n.監管",
"custom","n.海關;習慣",
"customary","adj.習慣上的",
"customer","n.顧客",
"cut","n./v.切",
"cute","adj.可愛的",
"cycle","n./v.週期;循環",
"cylinder","n.氣缸",
"daddy/dad","n.爹地",
"daily","adj./adv.每日的",
"dairy","n.乳品店",
"dam","n./v.水壩;築壩於...",
"damage","n./v.損害;毀壞",
"damages","n.損害賠償",
"dame","n.貴夫人",
"damp","adj.潮溼的",
"dance","n./v.舞蹈",
"dancer","n.舞者",
"danger","n.危險",
"dangerous","adj.危險的",
"dare","v.敢於...;膽敢",
"daresay","v.我想...;也許...",
"dark","adj./n.黑暗的",
"darken","v.使變黑",
"darkroom","n.暗房",
"darling","adj./n.親愛的",
"dart","n.鏢;急衝",
"dash","v.猛衝",
"data","n.資料",
"date","n.棗;棗椰樹",
"date","n./v.日期;約會",
"daughter","n.女兒",
"dawn","n./v.黎明;破曉",
"day","n 白天",
"daybreak","n.黎明",
"daydream","n./v.白日夢",
"daylight","n.日光",
"dazzle","v.使目眩",
"dead","adj.死去的;無效的",
"deadline","n.截止期限",
"deaf","adj.聾的",
"deafen","v.使聽不見",
"deal","n ./v.交易;處理",
"dean","n.學院院長;主任牧師",
"dear","adj./n./adv.親愛的",
"dearly","adv.深切地",
"debate","n./v.爭論",
"debt","n.債",
"debut","n.初次登臺",
"decease","n.死亡",
"deceive","v.欺騙",
"December","n.十二月",
"decency","n.莊重",
"decent","adj.得體的",
"deception","n.欺騙",
"deceptive","adj.虛偽的",
"decide","v.決定",
"decision","n.決定",
"decisive","adj.果斷的",
"deck","n.甲板",
"declaration","n.宣布;聲明",
"declare","v.宣告;聲明",
"decline","n./v.下降",
"decorate","v.裝飾",
"decoration","n.裝飾;裝溝",
"decrease","v./n.減少",
"decree","n.∕v.法令",
"dedicate","v.奉獻",
"deduction","n.推斷",
"deed","n.行為;功績",
"deem","v.視為",
"deep","adj./adv.深的",
"deepen","v.加深",
"deer","n.鹿",
"defeat","n./v.失敗;擊敗",
"defect","n.缺點",
"defend","v.防守;防衛",
"defense","n.防衛",
"defensive","adj.防護的;保衛的",
"deficiency","n.短缺",
"deficit","n.赤字",
"define","v.解釋;給...下定義",
"definite","adj.明確的;肯定的",
"defy","v.違抗",
"degree","n.度數;學位;等級",
"delay","n./v.拖延",
"delegate","v.派...為代表",
"delegation","n.委任",
"deliberately","adv.故意地",
"deliberation","n.深思",
"delicacy","n.精緻",
"delicate","adj.纖弱的;雅緻的",
"delicious","adj.美味可口的",
"delight","n./v.喻快",
"delightful","adj.令人愉快的",
"delinquent","adj.不法行為",
"deliver","v.遞送",
"delivery","n.投遞;交貨",
"demand","n./v.請求;要求",
"demanding","adj.苛責的",
"demo","n.(= demonstration)遊行示威",
"democracy","n.民主",
"democrat","n.民主主義者",
"democratic","adj.民主的",
"demon","n.惡鬼",
"demonstrate","v.證明;展示;遊行",
"demonstration","n.驗證;示威運動",
"denial","n.否認",
"denomination","n.教派",
"denote","v.意味著",
"denounce","v.指責",
"dense","adj.密集的;稠密的",
"dental","adj./n. 牙齒的;齒音(的)",
"dentist","n.牙科醫師",
"deny","v.否定;否認",
"depart","v.分離;啟程",
"department","n 部門;系",
"departure","n.分離;啟程",
"depend","v.依賴",
"dependent","adj.依賴性強的",
"depict","v.描繪",
"deplete","v.使枯竭",
"deploy","v./n.布署",
"deport","v.驅逐出境",
"deposit","n./v.存款;押金;放置",
"depreciate","v.跌價",
"depress","v.使沮喪",
"depression","n.沮喪",
"deprive","v.剝奪",
"depth","n.深度",
"descendant","n.後代",
"descent","n.下降",
"describe","v.描述",
"description","n.描寫;說明?",
"desert","n.沙漠",
"desert","v.拋棄",
"deserve","v.應得",
"design","n./v.設計",
"designate","v.指定;任命",
"designer","n.設計師",
"desire","n./v.慾望",
"desk","n.書桌",
"despairing","adj.自暴自棄的",
"desperate","adj.絕望的",
"despise","v.藐視",
"despite","prep.儘管",
"dessert","n.甜點;餐後點心",
"destination","n.目的地",
"destroy","v.摧毀",
"destruction","n.破壞",
"destructive","adj.毀滅性的",
"detach","v.分開",
"detail","n./v.細節",
"detailed","adj.詳細的",
"detain","v.拘留;耽擱",
"detect、偵測到","",
"deter","v.制止",
"detergent","n.洗潔劑",
"deteriorate","v.惡化",
"determination","n.決心",
"determine","v.決定;堅決",
"devalue","v.貶值",
"devastating","adj.毀滅性的",
"develop(ment)","v.(n.)發展",
"deviation","n.偏離",
"device","n.裝置",
"devil","n.魔鬼",
"devise","v.策劃",
"devote","v.將...奉獻給",
"devoted","adj.專心致志的",
"devotion","n.奉獻",
"diabetes","n.糖尿病",
"diagnose","v.診斷",
"diagnosis","n.診斷(法)",
"diagram","n.圖表(解)",
"dial","v./n.撥號;刻度盤",
"dialect","n.方言",
"dialogue","n.對話",
"diamond","n.鑽石",
"diaper","n.菱形花紋(布);尿布",
"diary","n.日記",
"dictate","v.使聽寫;命令",
"dictation","n. 聽寫;命令",
"dictator","n.獨裁者",
"dictionary","n.字典",
"die","v.死亡",
"diehard","n.頑固分子",
"diet","n.飲食;特種飲食",
"differ","v.不同於...",
"difference","n.差異",
"different","adj.不同的",
"differential","n.差別",
"differentiate","v.區分",
"difficult","adj.困難的",
"difficulty","n.困難",
"diffusion","n.擴散",
"dig","v. 挖;掘",
"digest","v.消化",
"digestion","n.消化作用",
"digital","adj./n.數字式的",
"dignify","v.使有尊嚴",
"dignitary","n.權貴",
"dignity","n.尊嚴",
"dilemma","n.困境",
"diligence","n.勤勉",
"diligent","adj.勤奮的",
"dilute","v./adj.稀釋(的)",
"dim","adj.朦朧的",
"dime","n.一角硬幣",
"dimension","n.尺寸;範圍",
"diminish","v.減少",
"dine","v.用餐",
"dinner","n.晚餐",
"dinosaur","n.恐龍",
"dip","v.浸;泡",
"diploma","n.學位證書",
"diplomacy","n.外交",
"diplomat","n.外交官",
"direct","adj./v.直接的;指向",
"direction","n.方向",
"director","n.主管;導演",
"directory","n.工商人名錄",
"dirt","n.污物;泥土",
"dirty","adj.污穢的",
"disability","n.無能",
"disable","v.使失去能力",
"disadvantage","n.不利之條件",
"disagree(ment)","v.(n)不同意",
"disappear","v.消失",
"disappoint(ment)","v.(n)使失望",
"disapproval","n 反對",
"disapprove","v.不贊成",
"disaster","n.災難",
"disastrous","adj.多災多難的",
"disbelieve","v.懷疑",
"disc/disk","n.圓盤;唱片",
"discard","v.摒棄",
"disciple","n.門徒",
"disciplinary","adj.紀律上的",
"discipline","n.紀律",
"disclose","v.露出",
"disclosure","n.揭露",
"disco","n.迪斯可舞廳/音樂",
"discomfort","n.不舒適",
"disconnect","v.使分離",
"discontent","n.不滿",
"discount","n./v.折扣",
"discourage(ment)","v.(n)使洩氣;使沮喪",
"discouraging","adj.令人洩氣的",
"discover","v.發現",
"discovery","n.發現",
"discreet","adj.考慮周到的",
"discrepancy","n.不一致",
"discretion","n.謹慎",
"discriminate","v.辨別",
"discrimination","n.歧視;區別",
"discuss","v.討論",
"discussion","n.討論",
"disease","n.疾病",
"disguise","n./v.偽裝",
"disgust","n./v.嫌惡;使厭惡",
"disgusting","adj.令人厭惡的",
"dish","n.碟;菜餚",
"dishonest","adj.不誠實的",
"dishwasher","n.洗碗機",
"disinterested","adj.公正的",
"dislike","不喜歡",
"dismantle","v.拆卸",
"dismay","v.使驚慌",
"dismiss","v.解散",
"disorder","n./v.使混亂;騷亂",
"disorganized","adj.雜亂無章的",
"dispatch","v.派遣;發送",
"dispel","v.驅散",
"dispensable","adj.可有可無的",
"disperse","v.使散開",
"displace","v.移開;取代",
"displacement","n.移位;取代",
"display","n./v.陳列",
"displease","v.使不快",
"disposable","adj.可丟棄的",
"disposal","n.處理;配置",
"dispose","v.處理;配置",
"disposition","n.配置",
"dispute","v./n.爭論",
"disqualify","v.取消資格",
"disregard","v.漠視",
"disrupt","v.擾亂",
"dissatisfaction","n.不滿",
"dissatisfy","v.使不快",
"dissident","adj./n.持異議的(人)",
"dissuade","v.勸阻",
"distance","n.距離",
"distant","adj.遙遠的",
"distinct","adj.有區別的",
"distinction","n.區別",
"distinctively","ad v.有特色地",
"distinguish","v.識別",
"distinguished","adj.卓越的",
"distort","adj.歪曲",
"distract","v.使分心",
"distress","n.悲痛",
"distribute","v.分配",
"distribution","n.分配;配給物",
"district","n.區域",
"distrust","v.不信任",
"disturb","v.妨礙;打擾",
"disturbing","adj.紛亂的",
"ditch","n.水道;渠道",
"dive","v./n. 跳水;潛水",
"diverse","adj.各種各樣的",
"diversify","v.使多樣化",
"diversion","n. 轉換",
"diversity","n.多樣性",
"divide","v./n.劃分;分歧",
"dividend","n.股息",
"divine","adj.神的;神聖的",
"division","n./v.分開;部門",
"divorce","n./v.離婚",
"divorcee","n.離了婚的女人",
"dizzy","adj.頭暈目眩的",
"dock","n./v.碼頭",
"doctor","n.醫生;博士",
"document","n./v.文件",
"documentary","n.記錄片",
"dodge","v./n.避開",
"dog","n.狗",
"dogmatic","adj.固執己見的",
"dole","n./v.救濟金;分發",
"doll","n.洋娃娃",
"dollar","n.(美、加等國)元",
"dolphin","n.海豚",
"domain","n.領域",
"domestic","adj.家庭的;國內的",
"dominant","adj.支配的",
"dominate","v.支配",
"donate","v.捐贈",
"donation","n.捐贈;捐贈物",
"done","adj.完成的",
"donkey","n.驢",
"donor","n.捐贈者",
"doom","n./v.厄運;注定",
"door","n.",
"dormitory/dorm","n.學生宿舍",
"dosage","n.劑量",
"dose","n.(藥物等的)一劑",
"dot","n./v.點;打點於",
"double","adj./adv./n./v.兩倍的;雙重地",
"doubt","v./n.懷疑;疑點",
"doubtful","adj.令人懷疑的",
"doubtless","adv.無疑地",
"dough","n.生麵團",
"doughnut","n.甜甜圈",
"dove","n. 鴿",
"down","prep./adj./adv./n./v.在...下方;下面的",
"downstairs","adv./adj./n.樓下",
"downtown","adv./adj./n.在城市的商業區;鬧區",
"downwards","adv.向下的",
"doze","v.打瞌睡",
"dozen","n.一打;十二個",
"draft","n./v.草稿",
"drag","v.拖曳",
"dragon","n.龍",
"dragonfly","n.蜻蜓",
"drain","v./n.排水",
"drainage","n.排水系統",
"drama","n.戲劇",
"dramatic","adj.戲劇性的",
"drape","v.覆蓋",
"drastic","adj.激烈的",
"draught","n./v.(=draft)草圖;草擬",
"draw","v.畫;拖拉",
"drawback","n.障礙;缺點",
"drawer","n.抽屜;拖曳者",
"drawing","n 素描;製圖",
"dread","n./v.害怕",
"dreadful","adj.可怕的",
"dream","n./v.夢;做夢",
"dress","n./v.女裝;打扮",
"drift","v./n.漂流;漂浮物",
"drill","n./v.鑽頭;鑽床",
"drink","n./ v.飲料;喝",
"drive","n./v.車程;駕駛",
"driveway","n.私用車道",
"drop","n./v.(一)滴;下降",
"dropout","n.輟學者",
"drought","n.乾旱",
"drown","v.把...淹死;淹沒",
"drowsy","adj.昏昏欲睡的",
"drug","n./v.藥品;毒品",
"drugstore","n.雜貨店",
"drum","n./v.鼓;打鼓",
"drunk","adj./n.喝酒醉的",
"drunkard","n.酒鬼",
"dry","adj./v.乾燥的;使變乾",
"dryer","n.吹風機",
"dual","adj./n.由兩部分構成的",
"dubious","adj.覺得可疑的",
"duck","n.鴨子",
"due","adj.到期的",
"duke","n.公爵",
"dull","adj.單調的;遲鈍的",
"dumb","adj.啞的;愚笨的",
"dump","v.傾倒",
"durable","adj.持久的",
"duration","n.持續",
"during","prep.在...期間",
"dust","n./v.灰塵;打掃",
"dutiful","adj.守本分的",
"duty","n.義務;稅",
"dwarf","n./adj.侏儒;矮小的",
"dwell","居住",
"dweller","n.居民",
"dye","v./n.染料;染色",
"dynamic","adj.動力的;動態的",
"dynamite","n.甘油炸藥",
"dynasty","n.朝代",
"each","adj./adv./pron.各個",
"eager","adj.熱切的",
"eagerly","adv.渴望地",
"eagle","n.鷹",
"ear","n.耳朵;聽力",
"early","adj./adv.早的",
"earn","v.賺取;贏得",
"earnest","adj.認真的",
"earnings","n.收入;工資",
"earplug","n.耳塞",
"earth","n.陸地;地球",
"earthquake","n.地震",
"earthworm","n.蚯蚓",
"ease","n./v.容易;減輕",
"east","adj./adv./n.東方的",
"Easter","n.復活節",
"eastwards","adv.向束",
"easy","adj./adv.容易的",
"eat","v.吃;喝",
"eccentric","adj./n.古怪的(人)",
"echo","n.回聲;回響",
"eclipse","n.(日、月)蝕",
"ecology","n.生態學",
"economic","adj.經濟上的",
"economical","adj.經濟的;節儉的",
"economically","adv.節約的",
"economics","n.經濟學",
"economist","n.經濟學者",
"economy","n.節約;經濟",
"edge","n./v.邊緣;使鋒利",
"edible","adj.可食的;食用的",
"edit","v.編輯",
"edition","n.發行數;版本",
"editorial","n.社論",
"educate","v.教育;教導",
"educated","adj.受過教育的",
"education","n.教育;教育學",
"educational","adj.有教育意義的",
"educator","n.教育工作者",
"eel","n.鰻",
"effect","n.結果;作用;影響",
"effective","adj.有效的",
"effectively","adv.有效地",
"efficiency","n.效率",
"efficient","adj. 有效率的",
"effort","n.努力",
"egg","n.蛋;卵細胞",
"ego","n.自我",
"eight","adj./n 八",
"eighteen","adj./n.十八",
"eighty","adj./n.八十",
"either","adj./pron.(兩者之中)任一的",
"either","adv.(在否定句中)也",
"elaborate","adj.精巧的;複雜的",
"elastic","adj.有彈性的",
"elbow","n.肘部;扶手",
"elder","adj./n.年齡較大的",
"elderly","adj.年長的",
"elect","v.選舉",
"election","n.選舉",
"elective","adj.由選舉產生的",
"electric","adj.電的",
"electrician","n.電工;電氣技師",
"electricity","n.電;電流",
"electronic","adj.電子的",
"electronics","n 電子學",
"elegant","adj.優雅的",
"element","n.元素",
"elemental","adj.自然力的",
"elementary","adj.基本的;初級的",
"elephant","n.象",
"elevate","v.使提高",
"elevator","n.電梯;升降機",
"eleven","n.十一",
"eligible","adj./n.有資格的",
"eliminate","v.消滅;排除",
"elite","n.精英",
"eloquent","adj.雄辯的",
"else","adv.其他;另外",
"elsewhere","adv.在別處",
"e-mail","n./v.電子郵件",
"embargo","v./n.禁止通商",
"embark","v.裝船;從事",
"embarrass(ment)","v.使出糢",
"embassy","n.大使館",
"embody","v.包括",
"embryo","n./adj.胚胎;未成熟的",
"emerge","v.浮現;顯露",
"emergency","n.緊急情況",
"emigrant","n.移民",
"emission","n.散發",
"emotion","n.情感;激動",
"emotional","adj.感情上的",
"emperor","n.統治者;皇帝",
"emphasis","n.強調",
"emphasize","v.強調",
"empire","n.帝國;王權",
"empirical","adj.憑經驗的",
"employ(ment)","v.(n)雇用",
"employee","n.雇員",
"employer","n.雇主",
"empty","adj./v.空的",
"enable","v.使能夠",
"enchant","v.使入迷",
"enclose","v.圍住;把(公文等)封入",
"encounter","v./n.遭遇;衝突",
"encourage(ment)","v.(n.)鼓勵",
"encouraging","adj.有激勵作用的",
"encyclopedia","n.百科全書",
"end","v./n.結束",
"endanger","v.使瀕臨危機",
"endless","adj.無盡的",
"endorse","v.贊同",
"endowment","n.天資",
"endurance","n.耐力",
"endure","v.忍受",
"enemy","n.敵人",
"energetic","adj.充滿活力的",
"energy","n.活力",
"enforce(ment)","v.(n〕實施",
"engagement)","v.(n)吸引;聘雇;訂婚",
"engine","n.發動機;引擎",
"engineer","n.工程師",
"engineering","n.工程",
"enhance","v.增加",
"enjoy(ment)","v.(n.)享受",
"enjoyable","adj.有樂趣的",
"enlarge(ment)","v.(n.)擴大",
"enlighten","v.敵發",
"enlist","v.入伍",
"enormous","adj.巨大的",
"enough","adj./adv./n.足夠的",
"enrich","v.使富裕",
"enroll","v.使加入;註冊",
"ensue","v.接著發生",
"ensure","v.確保",
"entail","v.使成為必要",
"enter","v.進入;參加",
"entertain(ment)","v.(n)娛樂",
"enthusiasm","n.熱心",
"enthusiastic","adj.有熱忱的",
"entire","adj.全部的",
"entrance","入口;進入",
"entry","n.進入",
"envelope","n.信封",
"envious","adj.妒嫉的",
"environment","n.環境;圍繞",
"environmental","adj.環境的",
"envisage","v.預見",
"envy","v.嫉妒;羨慕",
"enzyme","n.酵素",
"epic","n./adj.史詩(般的)",
"epigram","n.雋語",
"equal","adj./n./v.相等的",
"equality","n.相等;平等;均等",
"equip(ment)","v.(n〕裝備以",
"equivalent","adj./n.相同的",
"era","n.年代",
"erase","v.擦掉;抹去",
"eraser","n.擦除器(如橡皮擦)",
"erode","v.侵蝕",
"erosion","n.侵蝕",
"err","v.犯錯",
"errand","n.差事;任務",
"error","n.錯誤",
"escalator","n.電扶梯",
"escape","v./n.逃跑;逃避",
"escort","v./n.護衛(者)",
"essay","n.散文;論說文",
"essence","n.本質;精華",
"essential","adj.必要的;精華的",
"essentially","adv.基本上",
"establish(ment)","v.(n)建立",
"esteem","n./v.尊敬",
"estimate","n./v.估計;預計",
"eternity","n.永恆",
"ethic","n.倫理",
"ethics","n.倫理(學)",
"ethnic","adj.種族的",
"etiquette","n.禮節",
"evaluate","v.評估",
"evaluation","n.估算",
"evaporate","v.蒸發",
"eve","n.前夕",
"even","(1) adv.甚至;連",
"even","(2) adj./v.平坦的",
"evening","n.晚上",
"event","n.事件",
"eventual","adj.結果的",
"ever","adv.永遠",
"evergreen","n./adj.常青樹(的)",
"every","adj.每個",
"evidence","n./v.證據;物證",
"evident","adj.明顯的",
"evil","adj./n.邪惡的",
"evoke","v.喚起",
"evolution","n.演化(論)",
"evolve","v.使逐漸發展",
"exact","adj.精確的",
"exaggerate","v.誇大",
"exaggeration","n.誇張",
"examination/exam","n.考試",
"examine","v.檢查;調查",
"examinee","n.應試者",
"examiner","n.主考人;審查員",
"example","n.例子;樣品",
"exceedingly","adv.極其",
"excel","v.擅長",
"excellence","n.傑出;優點",
"excellent","adj.傑出的;卓越的",
"except","prep.除了...之外",
"exception","n.例外",
"exceptional","adj.例外的;特殊的",
"exchange","v./n.交換;交易",
"excite(ment)","v.(n.)使興奮",
"exclaim","v.驚叫",
"exclusively","adv.專門地",
"excuse","n./v.藉口;原諒",
"executive","adj.執行的",
"exempt","v./adj./n.豁免(的);被免除義務者",
"exercise","n./v.運動;鍛鍊",
"exhaust(1)","n.排出;排氣",
"exhaust","(2)v.汲乾;使精疲力盡",
"exhibit","v./n.展示",
"exhibition","n.展示會",
"exile","n./v.放逐",
"exist","v.存在;生存",
"existence","n.存在",
"exit","n./v.出口",
"exotic","adj./n.異國風情的",
"expand","v.展開;擴大",
"expansion","n.擴展;膨脹",
"expect","v.期望",
"expectation","n.期望",
"expedition","n.探險(隊)",
"expel","v.趕走",
"expend","v.花費",
"expensive","adj.昂貴的",
"experience","n./v.經驗",
"experiment","n./v.實驗",
"experimental","adj.實驗性質的",
"experimentation","n.實驗",
"expert","adj./n.熟練的;專家",
"expertise","n.專業知識",
"expire","v.期滿",
"explain","v.解釋",
"explanation","n.解釋",
"explicit","adj.清楚的",
"explode","v.爆炸",
"exploration","n.研究",
"explore","v.探索",
"explosion","n.爆炸;劇變",
"explosive","adj./n.爆炸性的",
"export","v./n.出口",
"expose","v.使暴露於",
"exposure","n.暴露;陳列",
"express","v. 表達",
"expression","n.表情;表示",
"extend","v.延伸",
"extension","n.伸長;延期",
"extensive","adj.廣大的",
"exterior","adj.外部的",
"extinct","adj.絕種的",
"extra","adj./adv./n.額外的",
"extract","v.拔出;摘錄",
"extracurricular","adj.課外的",
"extraordinary","adj.異常的",
"extravagance","n.浪費",
"extreme","adj./n.盡頭的;極限",
"eye","n./v.眼睛",
"eyesight","n.視力",
"eyewitness","n.目擊者",
"fable","n.寓言;虛構的故事",
"face","n./v.面孔;面臨",
"facial","adj.面部的;表面的",
"facility","n.設備;能力",
"fact","n.事實",
"factor","n.因素;代理人",
"factory","n.工廠",
"faculty","n.官能;技能",
"fade","v.凋謝;枯萎",
"fail","v./n.失敗;不及格",
"failure","n.失敗",
"faint","adj./v./n.頭暈的;暈倒",
"fair","(1) adj./adv.公平的",
"fair","(2) n.集市;商品展覽會",
"fairy","adj./n.仙女",
"faith","n.信念;諾言",
"faithful","adj.忠誠的",
"fake","adj./n./v.假的;偽造",
"fall","(1) v./n.落下",
"fall","(2) n.秋天;秋季",
"fame","n./v.聲譽",
"familiar","adj.熟悉的",
"family","n.家庭;家人",
"famous","adj.著名的",
"fan","n./v.扇子;煽動",
"fancy","adj./n./v.迷戀",
"fantastic","adj.想像中的",
"fantasy","n.空想;幻想",
"far","adj./adv.遙遠的",
"faraway","adj.遠方的",
"fare","n.車費;伙食",
"farewell","n.告別;告別詞",
"farm","n.農莊;農場",
"farmer","n.農人;農場經營者",
"farther","adv./adj.進一步地",
"fashion","n./v.時尚;風氣",
"fast","adj./adv.快速的",
"fasten","v.繫緊;集中注意力",
"fat","adj./n.肥胖的",
"fatal","adj.致命的",
"fate","n.命運",
"father","n.父親",
"fault","n./v.缺點;錯誤",
"favor","n./v.贊同;偏袒",
"favorable","adj.贊同的;適合的",
"favorite","adj./n.特別喜愛的",
"fax","v./n.傳真;傳真機",
"fear","n./v.害怕",
"feather","n.羽毛;鳥類",
"feature","n./v.特徵",
"February","n.二月",
"fee","n.酬金;服務費",
"feed","v./n.餵食",
"feel","v./n.摸;感覺",
"feet","n. [pl.]腳",
"fellow","adj./n.同伴的;伙伴",
"female","adj./n.女的;雌性的",
"fence","n./v.柵欄;防衛",
"ferry","n./v.擺渡;渡輸",
"fertile","adj.多產的;富饒的",
"fertilizer","n.肥料;促進發展者",
"festival","n.節日;歡樂",
"fever","n.發燒;狂熱",
"few","adj./n./pron.少數",
"fiction","n.小說;虛構的事",
"field","n.原野;欄位",
"fierce","adj.兇猛的;猛烈的",
"fifteen","adj./n.+五",
"fifth","adj./n.第五(的)",
"fifty","adj./n.五十",
"fight","v./n.打鬥;爭吵",
"fighter","n.鬥士;戰鬥機",
"figuron","v.外貌;計算;描述",
"file","n./v.檔案;把...歸檔",
"fill","二/n.裝滿",
"film","n./v.軟片;電影",
"final","adj.最後的",
"finance","n./v.財政",
"financial","adj.財政的;金融的",
"find","v./n. 尋找;得到",
"fine","adj.美好的;健康的",
"fine","n./v.罰金;處...以罰金",
"finger","n./v.手指;指出",
"finish","v./n.結束;完成",
"fire","n./v.火;開火",
"fire","v.激動;突然發怒",
"firm","adj/adv./v.堅決的;使穩固",
"first","adj./n adv.第一的",
"fish","n./v.魚類;釣魚",
"fisherman","n.漁民;漁夫",
"fist","n.拳頭;支配",
"fit","adj/n/v.適合的",
"five","adj/n.五",
"fix","v.修理;校準",
"flag","n./v.旗子",
"flame","n./v.火焰;燃燒",
"flash","n./v 閃爍;燈光",
"flat","adj/adv.平坦的",
"flatter","v.諂媚;奉承",
"flavor","n./v.味道;調味料",
"fiea","n.跳蚤",
"flee","v.逃走;逃離",
"flesh","n.肉體",
"flexible","adj.有彈性的",
"flight","n.航程;飛機",
"float","v.飄浮;搖擺",
"flock","n./v.群;聚集",
"flood","n./v.洪水;淹沒使氾濫",
"floor","n.地板",
"flour","n.麵粉",
"flow","n./v.漲潮;流動",
"flower","n./v.花朵;開花",
"flu","n.流行性感冒",
"fluent","adj.流暢的",
"fly","n.蒼蠅;蟲形魚鉤 v.飛行;逃跑",
"foam","n./v.泡沬",
"focus","n./v.焦點;集中",
"fog","n./v.霧氣;以霧籠罩",
"foggy","adj.多霧的",
"fold","n./v.摺疊;交疊",
"folk","n.(某一民族或社會階層中的)廣大成員",
"follow","v.跟隨",
"follower","n.追隨者;擁護者",
"following","adj./n.接著的",
"fond","adj. 喜歡的;愛好的",
"food","n.食物",
"fool","(1) n.傻瓜",
"fool","(2) v.愚弄;開玩笑",
"foolish","adj. 愚蠢的",
"foot","n.腳;足",
"forbid","v.禁止;阻止",
"force","n./v.強迫;力量",
"forecast","n./v.預測;預言",
"forehead","n.額;前額",
"foreign","adj.外國的;陌生的",
"foreigner","n.外國人",
"forest","n.森林",
"forever","adv.永遠",
"forge","v.打(鐵等)",
"forget","v.忘記",
"forgetful","adj.健忘的",
"forgive","v.原諒",
"fork","n./v.叉子;分岔",
"form","n./v.形狀;構成",
"formal","adj.正式的",
"formation","n.形成;組成",
"former","adj.先前的;從前的",
"formula","n.配方;客套話;準則",
"fort","n.堡壘",
"forth","adv.向前",
"fortunate","adj.幸運的",
"fortune","n.財產;好運",
"forty","adj./n.四十",
"forward","adj./v.向前;今後",
"forward(s)","adv.向前;今後",
"found","v.創立",
"foundation","n.建立;基金會",
"founder","n.創立者;締造者",
"fountain","n.噴泉;噴水池",
"four","adj./n.四",
"fourteen","adj./n.十四",
"fox","n.狐狸",
"fragment","n.碎片;未完成的部分",
"fragrance","n.芬芳",
"frame","n./v.架構",
"frank","adj.坦白的",
"free","adj.自由的",
"freedom","n.自由;獨立自主",
"freeway","n.高速公路",
"freeze","v.冰凍",
"freezer","n.冰箱",
"frequency","n.頻率;頻緊",
"frequent","adj.頻繁的",
"fresh","adj.新鮮的",
"freshman","n.新鮮人",
"Friday","n.星期五",
"friend","n.朋友",
"friendly","adj.友善的",
"friendship","n.友誼",
"frighten","v.使害怕",
"frog","n.青蛙",
"from","prep.始於;出自",
"front","adj./n.前面(的)",
"frost","n./v.霜;凍",
"frown","v./n.皺眉;表示不滿",
"fruit","n.水果;成果",
"frustrate","v.使挫敗",
"fry","v.油炸",
"fuel","n./v.燃料;加燃料",
"fulfill(ment)","v.(n)實現",
"full","adj.滿的;完全的",
"fun","n.娛樂;玩笑",
"function","n./v.功能;運行",
"functional","adj.起作用的",
"fund","n./v.資金",
"fundamental","adj.基礎的",
"funeral","n.葬儀",
"funny","adj.有趣的",
"fur","n.毛皮",
"furious","adj.狂怒的;猛烈的",
"furnish","v.裝備",
"furniture","n.傢俱",
"further","adj./adv./v.進一步的",
"furthermore","adv.此外",
"future","n./adj.未來;前途",
"gaiety","n.愉快",
"gain","n./v.獲得;獲利",
"galaxy","n.銀河系",
"gallery","n.畫廊;美術館",
"gallon","n.加侖",
"gamble","v.賭博;投機",
"game","n.遊戲;競賽",
"gang","n.惡少幫;阿飛黨",
"gap","n.缺口;峽谷;隔閡",
"garage","n.車庫",
"garbage","n.垃圾;廢物",
"garden","n./v.花園;從事園藝",
"gardener","n.園丁;花匠",
"garlic","n.大蒜",
"gas","n./v.氣體;瓦斯",
"gasoline","n.汽油",
"gasp","v.喘息",
"gate","n.大門;柵門",
"gather","v.收集;聚集",
"gauge","n.標準度量",
"gaze","v.凝視;注視",
"gear","n./v.齒輪;搭上齒輪",
"gear","n.汽車之排檔",
"Gemini","n.雙子座",
"gender","n.性別",
"gene","n.基因;遺傳因子",
"general","adj.一般的;普遍的",
"generalize","v.歸納",
"generate","v.產生;引起",
"generation","n.世代;產生",
"generator","n.發電機",
"generosity","n.慷慨",
"generous","adj.慷慨的",
"generously","ad v.慷慨地",
"genetics","n.遺傳學",
"genius","n.天賦;天才",
"gentle","adj.有教養的",
"gentleman","n.紳士",
"genuine","adj.真誠的",
"geographical","adj.地理學的",
"geography","n.地理學;地形",
"geometry","n.幾何學",
"germ","n.起源;細菌",
"gesture","n./v.做手勢;手勢",
"get","v.獲得;到達",
"ghetto","n.城市中猶太人的保留區",
"ghost","n.鬼;幽靈",
"giant","adj./n.巨人;巨大的",
"gift","n.禮品;天賦",
"gifted","adj.有天賦的",
"gigantic","adj.巨人似的;龐大的",
"gin","n.杜松子酒",
"ginger","n.",
"giraffe","n.長頸鹿",
"girl","n.少女",
"give","v.給;送給",
"glad","adj.高興的",
"glamour","n.魅力",
"glance","v.一瞥;掃視",
"glare","v.發強烈的光",
"glass","n.玻璃;玻璃杯",
"glasses","n.眼鏡",
"glassware","n.玻璃器皿",
"gleam","n.一絲光線",
"glide","v./n.滑行",
"glimpse","v./n.看一眼;瞥見",
"glitter","v.閃爍",
"global","adj.全球的",
"globe","n.地球;球狀物",
"gloomy","adj.黑暗的",
"glorious","adj.光榮的;輝煌的",
"glory","n./v.光榮;驕傲",
"glove(s)","n.手套",
"glow","v./n.發光;發熱",
"glue","n./v.膠水;黏牢",
"go","v.去;從事",
"goal","n.目標",
"goalkeeper","n.守門員",
"goat","n.山羊",
"god","n.上帝;造物主",
"goddess","n.女神;受尊崇的女子",
"gold","n./adj.黃金(色)",
"golden","adj.金色的;貴重的",
"goldfish","n.金魚",
"golf","n./v.高爾夫球",
"good","adj./n.好的;利益",
"goodbye","interj./n.再見",
"goodness","n.善良",
"goods","n.貨物",
"goose","n.鵝;傻瓜",
"gorgeous","adj.華麗的",
"gospel","n.主義",
"gossip","n./v.流言輩語",
"govern","v.統治;治理",
"government","n.政府;政體",
"governor","n.州長;主管",
"gown","n.長禮服;長抱",
"grab","v./n.抓取;奪取",
"grace","n./v.優雅",
"graceful","adj.優美的",
"gracious","adj.和藹的;雅致的",
"grade","n./v.級別;評分",
"gradual","adj.逐步的;平緩的",
"graduate","v./n.畢業;大學畢業生",
"graduation","n 畢業;畢業典禮",
"grain","n.穀粒;穀物",
"gram","n.(重量單位)克",
"grammar","n.文法",
"grammatical","adj.文法上的",
"grand","adj.雄偉的;重大的",
"grandchild","n.(外)孫子/女",
"granddaughter","n.孫女;外孫女",
"grandfather/grandpa","n.祖父;外祖父",
"grandmother/grandma","n.祖母;外祖母",
"grandson","n.孫子;外孫",
"grape","n.葡萄",
"grapefruit","n.葡萄柚",
"grasp","v./n.緊握;理解",
"grass","n.青草;草地",
"grasshopper","n.蝗蟲;蚱蜢",
"grate","n./v.壁爐;磨碎",
"grateful","adj.感謝的",
"gratitude","n.感激",
"grave","adj.重大的;嚴肅的",
"grave","n.墓穴;埋葬處",
"gravestone","n.墓碑",
"graveyard","n.墓地",
"gravity","n.重力;萬有引力",
"grease","n.油脂",
"greasy","adj.油膩的",
"great","adj.重大的",
"greatness","n.很多",
"greed","n.貪婪",
"greedy","adj.貪婪的",
"green","adj./n.綠色的",
"greenhouse","n.溫室",
"greet","v.問候",
"greeting(s)","n.問候語",
"gray","adj./n.灰色的",
"grief","n.悲傷;不幸",
"grieve","v.使悲傷",
"grill","v.烤",
"grim","adj.嚴厲的",
"grin","v./n.露齒而笑",
"grind","v.磨;碾;壓",
"groan","n./v.呻吟",
"grocery","n.食品雜貨店",
"groom","n.馬夫",
"ground","n.地面",
"group","n./v.群;團體;聚集",
"grove","n.樹叢",
"grow","v.發育;種植",
"growth","n.生長;發育",
"grumble","v.發怨言",
"guarantee","n./v.保證;擔保品",
"guard","n./v.保衛;守衛",
"guardian","n.監護人;管理員",
"guess","v./n.猜測",
"guest","n.客人",
"guidance","n.引導",
"guideline","n.指導方針",
"guilt","n.犯罪",
"guilty","adj.有罪的;內疚的",
"guitar","n.吉他",
"gulf","n.海灣;深淵",
"gum","n.樹脂;口香糖",
"gun","n.槍;砲",
"guy","n.傢伙;朋友",
"gymnasium/gym","n.體育館",
"gypsy","n.吉普賽",
"habit","n.習慣",
"habitual","adj.習慣的",
"hail","v.歡迎",
"hair","n.毛髮;頭髮",
"haircut","n.理髮",
"hairdo","n.髮型",
"hairdresser","n.美髮師",
"hairstyle","n.髮型",
"half","adj./adv./n.二分之一的",
"halftway","adj./adv.位於兩地中途的",
"hall","n.大廳;走廊",
"hallway","n.玄關;走廊",
"halt","v./n.停止;暫停",
"halt","v.猶豫",
"ham","n.火腿",
"hamburger","n.漢堡",
"hand","n./v.手;交給",
"handbook","n.手冊",
"handful","n.一把",
"handicap","n.障礙;不利條件",
"handkerchief","n.手帕;圍巾",
"handle","(1) n.柄;把手",
"handle","(2) v.觸摸;處理",
"handsome","adj.英俊的",
"handwriting","n 書寫",
"handy","adj.手邊的;巫巧的",
"hang","v.懸掛;吊死",
"hanger","n.衣架;絞刑執行者",
"happen","v.發生",
"happening","n.事件",
"happily","ad v.快樂的",
"happiness","n.快樂",
"happy","adj.快樂的",
"harassment","n.煩惱",
"harbor","n.海灣;港灣",
"hard","adj./adv.硬的;艱難的",
"harden","v.使變硬;使堅強",
"hardly","ad v.幾乎不",
"hardware","n.五金器具;硬體",
"hardy","adj.強壯的",
"hare","n.野兔",
"harm","n./v.損害;傷害",
"harmful","adj.有害的",
"harmony","n.協調",
"harness","n.馬具",
"harsh","adj.粗糙的;殘酷的",
"harshly","adv.刺耳地;嚴厲地",
"harvest","n./v.收穫",
"hasten","v.急忙",
"hastily","adv.急忙地",
"hasty","adj.匆忙的",
"hat","n.帽子",
"hatch","v.孵蛋;策劃",
"hate","v./n.憎恨",
"hatred","n.憎恨;敵意",
"haul","v.拖",
"hawk","n.鷹",
"hay","n.乾草",
"head","n.頭",
"headache","n.頭痛",
"heading","n.標題",
"headline","n.(報紙的)標題",
"head-on","adj./adv.正的",
"headquarters","n.總部",
"heal","v.治癒",
"health","n.健康",
"healthy","adj.健康的",
"heap","n.堆",
"hear","v.聽見",
"hearsay","n.謠言",
"heart","n.心臟;中心",
"heat","n./v.熱度;加熱",
"heater","n.暖氣機",
"heaven","n.天國;極樂之地",
"heavier","adj.重的",
"heavy","adj.沉重的",
"heavyweight","n./adj.重量級拳手;重的",
"hedge","n.樹籬",
"heel","n.腳後跟",
"height","n.高度",
"heighten","v.增高",
"heir","n.繼承人",
"heiress","n.女繼承人",
"helicopter","n.直昇機",
"helmet","n.頭盔;安全帽",
"help","n./v.幫助",
"helpful","adj.有幫助的",
"herald","n./v.使者;宣布 v.之將臨",
"herd","n.獸群",
"here","n./adv.這裡;在這裡",
"heroin","n.海洛英",
"heroine","n.女英雄",
"hesitantly","adv.遲疑地",
"heterosexual","adj.異性戀的",
"hierarchy","n.階級組織",
"hijack","v.劫取",
"hike","n.健行",
"hinder","v.妨礙",
"hint","n./v.暗示",
"hip","n.臀部",
"hire","v./n.租借;雇用",
"historian","n.歷史學家",
"historic", "adj.歷史上著名的",
"historical","adj.歷史的’",
"history","n.歷史",
"hit","v./n.打中;打擊",
"hobby","n.嗜好",
"hockey","n.曲棍球",
"hold","v./n.抓住;支撐",
"hole","n.孔;漏洞",
"holiday","n.假日",
"Hollywood","n.好萊塢",
"holy","adj.神聖的",
"home","n./adv.住家;家庭",
"homeland","n.祖國;家夠卜",
"homeless","adj.無家可歸的",
"homely","adj 家常的",
"homesick","adj.懷鄉病的",
"hometown","n.永久居留地",
"homework","n.家庭作業",
"homosexual","adj./n.同性戀",
"honest","adj.誠實的",
"honesty","n.誠實",
"honey","n.蜂蜜",
"honeymoon","n.蜜月假期",
"honor","n./v.榮譽;使...增光",
"honorable","adj.榮譽的",
"hook","n.勾子;陷阱",
"hop","v./n.跳過;跳躍",
"hope","v./n.希望",
"hopeful","adj.充滿希望的",
"horizon","n.地平線;眼界",
"hormone","n.荷爾蒙",
"horn","n.角;號;喇叭",
"horrible","adj.可怕的",
"horrify","v.使恐懼",
"horrifying","adj.駭人的",
"horror","n.恐怖",
"horse","n.馬",
"hose","n.長統襪及短襪",
"hospitable","adj.好客的",
"hospital","n.醫院",
"host","n./v.主人",
"hostage","n.人質",
"hostel","n.旅舍",
"hostess","n.女主人",
"hostile","adj.敵人的",
"hostility","n.敵意",
"hot","adj.熱的;熱情的",
"hotel","n.旅館;飯店",
"house","n./v.住宅",
"household","adj./n.家庭的",
"housekeeper","n.女管家",
"housewife","n.家庭主婦",
"housework","n.家事;家務",
"housing","n.住房建築",
"how","adv.如何;多少",
"however","adv./conj.然而",
"howl","v.號叫",
"hug","n./v.擁抱;抱住",
"huge","adj 龐大的",
"human","adj./n.人類的",
"humanity","n.人性;人文學科",
"humble","adj.謙恭的",
"humbly","adv.謙虛地",
"humid","adj.潮溼的",
"humidity","n.溼氣",
"humiliate","v.屈辱",
"humiliation","n.蒙羞",
"humor","n.幽默感",
"humorous","adj.幽默的",
"humorously","adv.幽默地",
"hundred","adj./n.一百",
"hunger","n.飢餓",
"hungry","adj.飢餓的;渴求的",
"hunt","v./n.追捕;打獵",
"hunter","n.獵人",
"hurl","v.用力投擲",
"hurrah","interj.歡呼聲",
"hurricane","n.颶風;暴風雨",
"hurry","n./v.急忙",
"hurt","n./v.受傷",
"husband","n.丈夫",
"hygiene","n.衛生",
"hymn","n.聖歌",
"hyphen","n.連字號",
"hypocrisy","n.虛偽",
"hypocrite","n.偽君子",
"hypothesis","n.假設",
"hysterical","adj.患歇斯底里症的",
"I.Q.","n.智力商數",
"ice","n.冰",
"iceberg","n.冰山",
"ID","n.身分證",
"idea","n.主意;觀念",
"ideal","adj./n.理想(的)",
"identical","adj.同一的",
"identification","n.識別;身分證明",
"identify","v. 鑑定;識別",
"identity","n.身分;同一性",
"ideological","adj.意識形態的",
"ideology","n.意識形態",
"idiom","n.慣用語",
"idiot","n.白癡",
"idle","adj.不做事的",
"idol","n.偶像",
"if","conj. 如果",
"ignorance","n.無知",
"ignorant","adj.無知的",
"ignore","v.不顧;忽視",
"ill","adj./adv.生病的",
"illegal","adj.違法的",
"illness","n.患病;身體不適",
"illogical","adj.不合理的",
"illuminate","v.照亮",
"illustrate","v.圖解;說明?",
"illustration","n.圖解;說明?",
"image","n.影像;形像",
"imaginary","adj.想像中的",
"imagination","n.想像力;創造力",
"imaginative","adj.想像的",
"imagine","v.想像",
"imitate","v.模仿",
"imitation","n.偽造",
"immediate","adj.立刻的",
"immense","adj.極大的",
"immigrant","n.移民",
"immigration","n.移居",
"immigrate","v.遷移",
"imminent","adj.逼近的",
"immoral","adj.不朽的",
"immune","adj.免除的",
"impact","n./v.衝擊;產生影響",
"impair","v.損害",
"impartial","adj.公平的",
"impatient","adj.不耐煩的",
"imperial","adj.帝國的",
"impersonal","adj.不受個人感情影響的",
"implement","v.實現",
"implicit","adj.暗示的",
"imply","v.暗示",
"impolite","adj 不禮貌的",
"import","n./v.進口",
"importance","n.重要性",
"important","adj.重要的",
"imposing","adj.氣勢雄偉的",
"impractical","adj.不切實際的",
"impress","v.使銘記",
"impression","n.印象",
"impressive","adj.令人印象深刻的",
"imprison","v.監禁",
"improve(ment)","v.(n.)改善",
"inability","n.無能力",
"inappropriate","adj.不適當的",
"incentive","n./adj.刺激",
"inch","n./v.英吋",
"incident","n.事件;插曲",
"incidentally","adv.附帶的",
"inclination","n.傾斜",
"include","v.包括",
"including","prep.包含·在內",
"income","n.收入",
"incomplete","adj.不完全的",
"inconvenient","adj.使人不便的",
"increase","n./v.增加",
"incredible","adj.驚奇的",
"incur","v.招致",
"indebted","adj. 負債的",
"indecision","n.無決斷力",
"indeed","adv.的確",
"independence","n.獨立",
"independent","adj.獨立的",
"independently","adv.獨立的",
"indicate","v.指示;象徵",
"indicator","n.指示者",
"indifference","n.不關心",
"indifferent","adj.對...不感興趣",
"indigestion","n.消化困難",
"indignant","adj.憤慨的",
"indignation","n.憤慨",
"indirect","adj.非直接的",
"indistinct","adj.模糊的",
"individual","adj./n.個別的;個人",
"indoor","adj.室內的",
"indoors","adv.在室內",
"indulge","v.使滿足",
"industrial","adj.工業的",
"industrialize","v.使工業化",
"industrious","adj.勤勉的",
"industry","n.工業",
"ineffective","adj.無效力的",
"inefficient","adj.無效率的",
"inevitable","adj.無可避免的",
"inexpensive","adj.不貴的",
"infant","n.嬰兒",
"infantry","n.步兵",
"infer","v.推斷",
"inference","n.推論",
"inferior","adj.低於...的",
"infinite","adj.無窮的",
"inflation","n.通貨膨脹",
"inflict","v.予以(打擊等)",
"influence","n./v.影響力;作用",
"influential","adj.有影響的",
"influenza","n.流行性感冒",
"inform","v.通知",
"informal","adj.非正式的",
"information","n.資訊",
"informative","adj 情報的",
"ingenuity","n.靈巧",
"ingredient","n.組成部分",
"inhabitant","n.居民",
"inherit","v.繼承",
"initial","(1) adj.最初的",
"initial","(2)n.字的起首字母",
"initially","adv.在起初",
"initiate","v.開始;使入會",
"inject","v.注射",
"injure","v.傷害",
"injury","n.傷害",
"injustice","n.不公正",
"ink","n./v.墨水;塗墨水於...",
"inn","n.小旅館;客棧",
"inner","adj.內在的",
"innocence","n.清白;無知",
"innocent","adj.清白的;無知的",
"innovation","n.革新;創新",
"input","n./v.輸入",
"inquisitive","adj.好管閒事的",
"insane","adj.瘋狂的",
"insect","n.昆蟲",
"insecure","adj.不安全的",
"insert","v.插入",
"inside","prep./adj./adv./n.在...的裡面;內部的",
"insist","v.堅持",
"inspect","v.檢查",
"inspection","n.檢查",
"inspiration","n.靈感",
"inspire","v.鼓舞",
"inspiring","adj.令人鼓舞的",
"instability","n.不穩定",
"installation","n.安置",
"instance","n.例子",
"instant","adj.立刻的",
"instantly","adv.立即",
"instead","adv.反而",
"instinct","n.本能;直覺",
"institutional","adj.制度(上)的",
"instruct","v.指導",
"instruction","n.教導;指示",
"instructor","n.指導者;教練",
"instrument","n.儀器;樂器",
"insult","n./v.羞辱",
"insulting","adj.侮辱人的",
"insurance","n.保險",
"intact","adj.未觸動的",
"integral","adj.構成整體所需要的",
"integrity","n.廉正",
"intellect","n.理解力",
"intelligence","n.智慧;情報工作",
"intelligent","adj.聰明的",
"intelligible","adj.易了解的",
"intend","v.打算",
"intense","adj.強烈的",
"intensify","v.強化",
"intensity","n.強烈;強度",
"intensive","adj.加強的",
"intent","n.目的",
"intention","n.意圖;目的",
"interact","v.互動",
"interaction","n.互動",
"intercept","v.中途阻止",
"interchangeable","adj.可交換的",
"intercourse","n.交際",
"interest","n./v.利益;興趣",
"interesting","adj.有趣的",
"interfere","v.妨礙;干預",
"interference","n.阻礙;干預",
"interior","n./adj.內部(的)",
"intermediate","adj.中問的",
"internal","adj.內部的",
"international","adj.國際的",
"Internet","n.網際網路",
"interpret","v.解釋;翻譯",
"interpretation","n.解釋;翻譯",
"interpreter","n.口譯者",
"interrupt","v.打斷",
"interruption","n.阻礙;干擾",
"intersection","n.橫切",
"interview","n./v.會見;面試",
"intimate","adj.親密的;熟悉的",
"intimidate","v.脅迫",
"intonation","n.語調",
"intricate","adj.複雜的",
"intrigue","v./n.密謀;陰謀",
"intriguing","adj. 有趣的;有魅力的",
"introduce","v.介紹;提出",
"introduction","n.介紹;引言",
"intuition","n.直覺",
"invade","v.侵略;侵犯",
"invasion","n.入侵",
"invent","v.發明;創造",
"invention","n.發明;創造力",
"inventor","n.發明者",
"invest(ment)","v.(n.)投資;投入",
"investigate","v.調查;研究",
"investigation","n.調查;研究",
"investigator","n.調查者",
"investor","n.投資者",
"invisible","adj.看不見的",
"invitation","n.邀請",
"invite","v.邀請",
"inviting","adj.吸引人的",
"invoke","v.祈求幫助或保護",
"involve(ment)","v.(n)使捲入",
"inward","adj.在內的",
"iron","n./adj.鐵;熨斗",
"ironic","adj.諷剌的",
"irony","n.反語",
"irregular","adj.不合常規的",
"irrelevant","adj.不切題的",
"irresponsible","adj.不須負責任的",
"irritable","adj.急躁的",
"irritate","v.激怒",
"island","n.島",
"isolate","v.使孤立",
"isolated","adj.孤立的",
"isolation","n.隔離",
"Italian","n.義大利文",
"itch","v.發癢",
"item","n.項目",
"ivory","adj./n.似象牙的;象牙(質)",
"jacket","n.夾克",
"jail","n./v.拘留所",
"jam","n.果醬;堵塞",
"January","n.一月",
"Japanese","n.日本人",
"jar","n.罐;罈;瓶",
"jar","n.刺耳的聲音",
"jasmine","n.茉莉",
"jaw","n.頜;顎;下巴",
"jaywalk","v.不遵守交通規則而穿越馬路",
"jazz","n.爵士樂",
"jealous","adj.嫉妒的",
"jeans","n.牛仔褲",
"jeep","n.吉普車",
"jelly","n.果凍",
"jerk","v.猛然一推",
"jet","n.噴射機",
"jewel","n.珠寶",
"jewelry","n.(總稱)珠寶",
"job","n.工作",
"jog","n./ v.輕搖;慢跑",
"jog","v.輕撞",
"join","v.參加",
"joke","n./v.玩笑",
"journal","n.雜誌;期刊",
"journalism","n.新聞業",
"journalist","n.新聞記者",
"journey","n.旅行",
"joy","n.歡樂",
"joyful","adj.高興的",
"joyous","adj.快樂的",
"judge(ment)","n./v.法官",
"judicial","adj.法庭的",
"juice","n.果汁",
"July","n.七月",
"jump","n./v.跳躍",
"junction","n.連接",
"June","n.六月",
"jungle","n.叢林",
"junior","n./adj.較年少者",
"junk","n.廢物;垃圾",
"just","adj.公平的",
"just","adv.僅僅",
"justice","n.公平;正義",
"justification","n.理由",
"justly","adv.合理的",
"juvenile","adj.少年的 K",
"kangaroo","n.袋鼠",
"keen","adj.熱衷的",
"keenly","adv 激烈地",
"keep","v.保存",
"ketchup","n.調味蕃茄醬",
"kettle","n.水壺",
"key","adj.關鍵的",
"key","n.鑰匙;關鍵",
"keyboard","n.鍵盤",
"kick","v.踢;反衝",
"kid","n.小孩",
"kidnap","v.綁架",
"kidney","n.腎",
"kill","v.殺死;使終止",
"killer","n.殺人者",
"kind","adj.親切的",
"kind","n.種類;性質",
"kindergarten","n.幼稚園",
"kindly","adj.友善的",
"kindness","n.親切",
"king","n.國王",
"kingdom","n.王國;王權",
"king-size","adj.特大號的",
"kiss","n./v.吻",
"kit","n.所有的裝備",
"kitchen","n.廚房",
"kite","n.風箏",
"kitten","n.小貓",
"knee","n.膝蓋",
"kneel","v.跪下",
"knife","n.刀",
"knight","n.騎士;爵士",
"knit","v.編織;接合",
"knob","n.節;瘤",
"knock","n./v.敲;打",
"knot","n./v.(繩等的)結",
"know","v.知道",
"know-how","n.技能",
"knowledge","n.知識",
"knowledgeable","adj.博識的 L",
"lab","n.實驗室",
"label","n./v.標籤",
"labor","n./v.勞動;勞工",
"laboratory/lab","n.實驗室",
"lace","n.鞋帶;花邊",
"lack","v./n.缺少",
"lad","n.少年",
"ladder","n.梯子",
"lady","n.女士",
"lake","n.湖",
"lama","n.喇嘛",
"lamb","n.小羊",
"lament","v.悲傷",
"lamp","n.燈",
"land","n.土地",
"land","v.使登陸",
"landing","n.登陸",
"landlady","n.女房束",
"landlord","n.地主",
"landmark","n.陸標",
"landmine","n.地雷",
"landslide","n.山崩",
"lane","n.小路;巷弄",
"language","n.語言",
"lantern","n.燈籠;街燈",
"lap","n.膝部",
"large","adj.大的",
"largely","adv.大部分",
"laser","n.雷射",
"last","adj./adv./n./pron. 最後的;最近的",
"last","v.持續",
"late","adj./adv.遲的;晚的",
"lately","adv.最近",
"latest","adj.最近;最遲",
"latitude","n.緯度",
"latter","adj./n.後面的;後半的",
"flatter","adj.近來的",
"laugh","n./v.笑",
"laughter","n.笑",
"launch","v./n.發射;投擲",
"laundry","n.洗衣店",
"law","n.法律",
"lawful","adj.法律上的",
"lawmaker","n.立法者",
"lawn","n.草坪",
"lawnmower","n.除草機",
"lawsuit","n.訴訟",
"lawyer","n.律師",
"lay","v.放;安排;下蛋",
"layman","n.門外漢",
"lazy","adj.懶散的",
"lead","v.領導",
"leader","n.領袖",
"leadership","n.指揮;統率力",
"leaf","n.葉子;一張或一頁",
"leak","v.漏",
"leakage","n.洩漏",
"lean","v.傾斜;倚靠",
"leap","n./v.跳躍;舞",
"learn","v.學習",
"learned","adj.有學問的",
"lease","n.租約",
"leather","n.皮革",
"leave","n./v.休假;許可;離開",
"lecture","n./v.授課",
"lecturer","n.講演者;講師",
"leg","n.腿",
"legacy","n.遺產",
"legal","adj.法律上的;合法的",
"legally","adv.法律上",
"legend","n.傳說",
"leisure","adj./n.空閒的",
"leisurely","adj./adv.悠然的",
"lemon","n.檸檬",
"lemonade","n.檸檬水",
"lend","v.把...借給",
"length","n.長度;期間",
"lengthy","adj.冗長的",
"lengthen","v.使延長",
"lens","n.鏡片;眼球的水晶體",
"lesbian","n.女同性戀者",
"less","adj./adv.較少的/地",
"lesser","adj./adv.較小的",
"lesson","n.功課;課程",
"let","v.允許;讓",
"letter","n.信;字母",
"level","n.高度;層;水準",
"liability","n.有義務",
"liar","n.說謊的人",
"liberal","adj.心胸寬闊的",
"liberate","v.解放",
"liberation","n.解放",
"liberty","n.自由",
"librarian","n.圖書館員",
"library","n.圖書館",
"license","n./v.許可;執照",
"lick","n./v.舐;(火)蔓延",
"lid","n.蓋子;眼瞼",
"lie","n./v.躺臥",
"lieutenant","n.陸軍中尉",
"life","n.生命",
"lifeboat","n.救生艇",
"lifeguard","n.救生員",
"lifelong","adj.終生的",
"lifetime","n.一生",
"lift","n./v.舉起",
"light","adj./n./v.明亮的",
"lighten","v.照亮",
"lightning","n.閃電;閃光",
"lightweight","n./adj.平均重量以下;不重要的人",
"like","prep.像;和·一樣",
"like","n./v.喜歡",
"likelihood","n.可能性",
"likely","adj.很可能的",
"likewise","adv.同樣的",
"liking","n 愛好",
"lily","n.百合花",
"limb","n.肢;臂",
"limit","n./v.限制",
"limitation","n.限制",
"limited","adj.被限制的",
"limp","v.跛行",
"line","n./v.線條;畫線於",
"linen","n.亞麻布製品",
"linger","v.逗留",
"lingering","adj.持久的",
"linguist","n.精通數種外國語言者",
"linguistics","n.語言學",
"link","n./v.連結;環結",
"lion","n.獅子",
"lip","n.嘴唇",
"lipstick","n.口紅",
"liquid","n.液體",
"liquidation","n.清償",
"liquor","n.含酒精的飲料",
"list","n./v.表;把...編列成表",
"listen","v.聽;聽從",
"listener","n.聽眾",
"literacy","n.識字;讀寫能力",
"literary","adj.文學的",
"literate","adj.文雅的",
"literature","n.文學",
"litter","n.雜物;垃圾",
"little","adj.小的",
"live","adj.活的",
"live","v.活著;過(生活)",
"livelihood","n.生計",
"lively","adj.有生氣的",
"liver","n.肝臟;居住者",
"livestock","n.家畜",
"living","adj.活著的",
"lizard","n.蜥蜴",
"load","n./v.裝載;負擔",
"loaf","n.一條麵包;遊蕩",
"loan","n./v.借出",
"lobby","n.門廊;休息室",
"lobster","n.龍蝦",
"local","adj.地方性的",
"locate","v.使...座落於",
"location","n.選定路線",
"lock","n./v.鎖",
"locomotive","adj.移動的",
"loft","n.閣樓",
"log","n.原木",
"logic","n.邏輯",
"logical","adj.邏輯的",
"logo","n.(=logotype)標識;商標",
"lonely","adj.寂寞的",
"lonesome","adj.寂寞的",
"long","v.渴望",
"longevity","n.長壽",
"look","n./v.看",
"loom","v.隱約的或威脅性的出現",
"loop","n.環形花狀",
"loose","adj.鬆散的",
"loosely","adv.不嚴格地;寬鬆地",
"loosen","v.放鬆",
"lord","n.統治者",
"lorry","n.載貨卡車",
"lose","v.丟失",
"loser","n.失敗者",
"loss","n.喪失;遺失",
"lot","n.很多",
"lotion","n.外用藥水;化妝水",
"lottery","n.獎券;彩票",
"lotus","n.蓮;忘憂樹",
"loud","adj.大聲的",
"loudspeaker","n.揚聲器",
"lounge","n./v.休息室;懶洋洋地坐或站",
"lousy","adj.不潔的;卑鄙的",
"lovable","adj.可愛的",
"love","v./n.愛;喜歡",
"lovely","adj.可愛的",
"lover","n.戀人",
"loving","adj.愛的",
"low","adj.低的",
"lower","adj./v.較低的;降下",
"loyal","adj.忠誠的",
"loyalty","n.忠誠",
"lubricant","n.潤滑劑",
"luck","n.運氣",
"lucky","adj.幸運的",
"lucrative","adj.可獲利的;賺錢的",
"luggage","n.行李",
"luminous","adj.明亮的;清楚的",
"lunar","adj.陰曆的;月亮的",
"lunch","n.午餐",
"luncheon","n.午餐;午宴",
"lung","n.肺",
"luxury","n.奢侈",
"lyric","n.抒情詩",
"machine","n.機器",
"machinegun","n.機關槍",
"machinery","n.機械裝置",
"mad","adj 發瘋的;狂熱的",
"madam","n.夫人;太太;小姐",
"magazine","n.雜誌;期刊",
"magic","adj./n.有魔力的;魔法",
"magical","adj.魔法的",
"magician","n.魔術師",
"magnet","n.磁鐵",
"magnetic","adj.磁鐵的;帶磁性的",
"magnificent","adj.壯麗的",
"magnify","v.擴大;誇大;誇獎",
"maid","n.少女",
"maiden","n.少女",
"mail","n./v.郵件;郵寄",
"main","adj.主要的",
"mainland","n.大陸",
"mainstream","n.主流",
"maintain","v.保持",
"majestic","adj.莊嚴的",
"majesty","n.威嚴;王權",
"major","adj./n./v.主要的;主修",
"majority","n.多數",
"make","v.做;到達;獲得",
"makeup","n.裝扮;化妝品",
"malaria","n.瘧疾",
"male","adj./n.男性的;雄的",
"malicious","adj.存心不良的",
"mall","n.大規模購物中心",
"man","n.男人",
"manage(ment)","v.(n)管理",
"manageable","adj.能處理的",
"manager","n.經理;管理人",
"mandarin","n.國語",
"manhood","n.成年;男子氣質",
"mankind","n.人類",
"manifestation","n.顯示;發表",
"manifesto","n.宣言;聲明",
"manipulate","v.操縱",
"manner","n.方法;態度",
"manners","n.禮貌;習俗",
"manpower","n.人力",
"mansion","n.大廈;官邸",
"manufacture","n./v.製造業;製造",
"manufacturer","n.製造者",
"manuscript","n.草稿;手稿",
"many","adj./pron. 許多",
"map","n./v.地圖",
"marathon","n.馬拉松賽跑",
"marble","n.大理石",
"March","n.三月",
"march","v.行軍;走過",
"mare","n.母馬;母驢",
"margin","n.邊綠",
"marginal","adj.空白處的;邊際的",
"mark","n./v.痕跡;記號",
"market","n./v.市場",
"marriage","n.結婚",
"married","adj.已婚的",
"marry","v.娶;嫁",
"Mars","n.戰神;火星",
"marshal","n.元帥",
"martial","adj.軍事的",
"marvel","n.令人驚奇或驚喜的事物",
"marvelous","adj.奇妙的",
"masculine","adj.男性的;陽性的",
"mask","n./v.面具",
"mass","n.大量",
"massacre","n.大屠殺",
"massage","n.按摩",
"master","n.主人;雇主",
"master","v.做...主人;精通",
"masterpiece","n.傑作;名著",
"mastery","n.控制;熟練",
"mat","n.墊;草簾",
"match","n./v.比賽;競賽",
"mate","n.同伴",
"material","n.材料",
"mathematical","adj.數學的",
"mathematics/math","n.數學",
"matter","n.事情;問題",
"matter","v.有關係;要緊",
"mature","adj.成熟的",
"maturity","n.成熟;完成",
"maximum","n.最大限度",
"May","n.五月",
"maybe","adv.可能",
"mayor","n.市長;鎮長",
"meadow","n.草地",
"meal","n.一餐",
"mean","adj.吝嗇的",
"mean","v.表示...的意思",
"meaning","n.意義;重要性",
"meaningful","adj.意味深長的",
"meaningless","adj.無意義的",
"means","n.手段;方法",
"meanwhile","adv.其問;同時",
"measles","n.麻疹",
"measure(ment)","v./n.測量",
"meat","n.(食用的)肉",
"mechanical","adj.機械的",
"mechanics","n.力學;機械學",
"medal","n.獎章;紀念章",
"media","n.媒體",
"medical","adj.醫學的",
"medication","n.藥物治療",
"medicine","n.藥;醫學",
"medieval","adj.中古的;中世紀的",
"meditation","n.沉思;冥想",
"medium","n.中間;媒介",
"meek","adj.溫順的",
"meet","v.遇見",
"meeting","n.會議",
"melancholy","n./adj.悲哀;憂鬱",
"mellow","adj.香醇的;柔和的;老練的",
"melody","n.旋律",
"melon","n.甜瓜",
"melt","v.融化",
"member","n.成員",
"membership","n.會員(資格)",
"memo","n.備忘錄",
"memorable","adj.值得紀念的",
"memorial","adj./n.紀念的;紀念品",
"memorize","v.背誦;記憶",
"men","n. man 的複數",
"menace","n.危險;威脅",
"mental","adj.精神的",
"mentality","n.智力;心性",
"mentally","adv.智力不足的;心理有缺陷的",
"mention","n./v.提及",
"mentor","n.導師",
"menu","n.菜單;項目單",
"merchant","n.商人",
"mercy","n.慈悲",
"mere","adj.僅僅的",
"merit","n.價值;長處",
"merry","adj.歡樂的",
"mess","n.紊亂",
"message","n.消息",
"messenger","n.使者;信差",
"metal","n.金屬",
"metaphor","n.",
"metaphysical","adj.玄學的;形而上學的",
"meter","n.測量儀表;公尺",
"method","n.方法",
"microphone","n.麥克風",
"microwave","n.微波",
"middle","adj./n.中問的;中部",
"midnight","n.午夜",
"mighty","adj.強大的;偉大的",
"migration","n.遷移",
"mild","adj.溫和的;順口的",
"mileage","n.哩數",
"militant","adj.好戰的",
"military","adj./n.軍事的;軍隊",
"milk","n./v.牛奶",
"mill","n./v.磨坊;碾碎",
"milligram","n.毫克",
"million","n.百萬",
"millionaire","n.百萬富翁",
"mind","n./v.頭腦;注意;留意",
"mine","n.礦;礦山",
"miner","n.礦工",
"mingle","v.混合",
"minimal","adj.最低程度的",
"minimum","n.最低限度;極小",
"minister","n.部長;外交使節",
"ministry","n.牧師之職;內閣",
"minor","adj./n.較小的;次要的",
"minority","n.少數",
"minus","prep./adj./n.負的",
"minute","n.分鐘",
"miracle","n.奇蹟",
"miraculously","adv.神奇地",
"mirror","n./v.鏡子;反映",
"misbehave","v.做不正當的事",
"miscellaneous","adj.各式各樣的",
"mischief","n.危害;淘氣",
"misconduct","n.不規矩的行為;不當的處理",
"miserable","adj.不幸的",
"misery","n.不幸;悲慘",
"misfortune","n.壞運氣;不幸",
"mishap","n.災難",
"mislead","v.誤導",
"miss","v.錯過;想念",
"missile","n.飛彈",
"missing","adj.行蹤不明的",
"mission","n.使節團;使命",
"missionary","n.傳教士",
"mist","n.霧;迷濛",
"mistake","n.錯誤;過失",
"mistress","n.女主人",
"misunderstand","v.誤會",
"misunderstanding","n.誤解",
"mix","v.混和",
"mixture","n.混合物",
"moan","v./n.呻吟(聲)",
"mob","n.烏合之眾;無秩序的民眾",
"mobile","adj./n 易;汽車",
"mock","v.愚弄;嘲弄",
"model","n.模型;模範",
"moderate","adj.中等的",
"modem","adj.現代的",
"modernization","n.現代化",
"modernize","v.使現代化",
"modest","adj.謙虛的",
"modesty","n.謹慎;謙遜",
"moist","adj. 潮溼的",
"moisture","n.溼氣;水份",
"molecule","n.分子",
"moment","n.片刻",
"momentary","adj.短暫的",
"momentum","n.動量;動向",
"Monday","n.星期一",
"monetary","adj.貨幣的",
"money","n.錢",
"monitor","n./v.監視器;監控",
"monitor","n.級長",
"monk","n.僧侶;修道士",
"monkey","n.猴子",
"monopoly","n.壟斷;專利",
"monotony","n.單調",
"monster","n.怪物",
"monstrous","adj.怪物的;恐怖的",
"month","n.月",
"monthly","adj.每月一次的",
"monument","n.紀念碑;紀念館",
"mood","n.情緒",
"moon","n.月亮",
"mop","n./v.拖把",
"moral","adj.道德上的",
"morale","n.士氣",
"more","adj./adv.更多的",
"moreover","adv.並且;此外",
"morning","n.早晨",
"mortal","adj.必死的",
"mortar","n.灰泥",
"mosquito","n.蚊",
"most","adj./adv.最大程度地",
"mostly","adv.大部分",
"motel","n 汽車旅館",
"mother/mom","n.母親;媽媽",
"motherhood","n.母性",
"motif","n.主旨",
"motion","n.運動;動機",
"motivate","v.給與動機",
"motivation","n.動機",
"motor","n.馬達;原動力",
"motorcycle","n.機車",
"motto","n.箴言;座右銘",
"mountain","n.山",
"mountainous","adj.多山的",
"mourn","v.悲悼",
"mouse","n.鼠;滑鼠",
"mouth","n.嘴;口",
"movable","adj.可動的",
"move(ment)","v.(n.)使移動;使感動",
"movie","n.電影",
"Mrs.","n.太太",
"much","adj./adv./n.許多的",
"mud","n.泥",
"muddy","adj.泥的;骯髒的",
"multiple","adj.多重的",
"murder","n./v.謀殺",
"murderer","n.兇手",
"murmur","n./V.私語",
"muscle","n.肌肉",
"muscular","adj.肌肉的",
"muse","n.繆思女神;詩興",
"museum","n.博物館",
"mushroom","n./v.香茹;蘑蘋狀的束西",
"music","n.音樂",
"musical","adj./n.音樂的",
"musician","n.音樂家",
"mustache","n.鬚",
"mustard","n.芥末",
"mute","adj.沉默的",
"mutton","n.羊肉",
"mutual","adj.相互的",
"mysterious","adj.神祕的",
"mystery","n.神祕的事物",
"mythology","n.神話學",
"nag","v.不斷地挑剔",
"nail","n./v.釘子;指甲",
"naive","adj. 天真的;樸素的",
"naked","adj. 裸體的",
"name","n./v.名字",
"nap","n./v.打盹",
"napkin","n.餐巾",
"narrate","v.敘述",
"narrative","n.故事",
"narrow","adj./v.狹窄的;限制",
"nation","n.國家;民族",
"national","adj.國家的",
"nationality","n.國籍",
"native","adj./n.本國的",
"natural","adj.自然的",
"naturally","adv.自然地",
"nature","n.自然",
"naughty","adj.頑皮的",
"navy","n.海軍",
"nearby","adj./adv.附近的",
"nearly","adv.幾乎",
"nearsighted","adj.近視的",
"neat","adj.整潔的",
"necessary","adj.必需的",
"necessity","n.必需;必需品",
"neck","n.頸;脖子",
"necklace","n.項鍊",
"necktie","n.領帶;領結",
"need","v./n.需要",
"needle","n.針",
"needy","adj.非常貧窮的",
"negative","adj./n.否定的",
"neglect","v.忽視",
"negligible","adj.不重要的",
"negotiable","adj.可協商的",
"negotiate","v.談判",
"neighbor","n.鄰居",
"neighborhood","n.鄰近地區",
"neither","adj./adv./pron./conj. 兩者都不",
"nephew","n.姪兒;外甥",
"nerve","n.神經",
"nervous","adj.緊張的",
"nest","n./v.築巢",
"net","n./v.網;用網捕",
"never","adv.永不",
"nevertheless","adv.仍然;不過",
"new","adj.新的",
"newcomer","n.新來者",
"newlywed","n.新婚的人",
"news","n.新聞",
"newscaster","n.新聞播報員",
"newspaper","n.報紙",
"nice","adj.好的",
"nickel","n.鎳",
"nickname","n.綽號",
"niece","n.姪女;外甥女",
"night","n.夜",
"nightingale","n.夜鶯",
"nightmare","n.惡夢",
"nine","adj./n.九個",
"nineteen","adj./n.十九",
"ninety","adj./n.九十",
"nitrogen","n.氮",
"nobility","n.高貴;高尚",
"noble","adj.高貴的;貴族的",
"nod","n./v.點頭;打瞌睡",
"noise","n.噪音",
"noisy","adj.吵鬧的",
"nominal","adj.名義上的;不重要或無價值的",
"nominate","v.提名某人;指派",
"nomination","n.提名;任命",
"nominee","n.被提名人",
"none","pron./adv.一點兒也沒",
"nonetheless","adv./conj.但是;仍然",
"nonsense","n.胡說",
"noodle","n.麵條",
"noodle","n.笨人;傻瓜",
"noon","n.中午",
"nor","conj.也不",
"normal","adj.正常的",
"north","adj./adv/n.北方",
"northern","adj.北方的",
"nose","n.鼻;嗅覺",
"notable","adj.顯著的",
"notably","adv.顯著地;尤其",
"note","n./v.筆記;注意",
"noteworthy","adj.值得注意的",
"notice","n/v.公告;注意;通知",
"noticeable","adj.顯明的",
"notify","v.通知",
"notorious","adj.惡名昭彰的",
"noun","n.名詞",
"nourish(ment)","v.(n)滋養",
"novel","adj.新奇的",
"novel","n.長篇小說",
"novelist","n.小說家",
"novelty","n.新鮮;新奇",
"November","n.+一月",
"now","nJadv.現在",
"nowadays","adj.現在;今日",
"nuclear","adj.核子的",
"nucleus","n.中心;核心",
"nude","adj.裸體的",
"nuisance","n.討厭的事物、人、行為等",
"number","n./v.數字;給...號碼",
"numerical","adj.數字的;表示數量",
"numerous","adj.許多的",
"nun","n.修女;尼姑",
"nurse","n.護士",
"nursery","n.育嬰室",
"nurture","n.養育;教養",
"nut","n.堅果;瘋子",
"nutrition","n.營養;營養學",
"oak","n.橡樹",
"oasis","n.綠洲",
"oath","n.誓約;詛咒",
"obedient","adj.順從的;孝順的",
"obey","v.服從;聽從",
"obituary","n.訃聞",
"object","n.物體;目標",
"object","v.反對",
"objection","n.反對",
"objective","adj.客觀的",
"oblong","n./adj.長方形;長方形的",
"obscure","adj./n.使暗;使不明顯;暗的;不明顯的",
"observant","adj.善於觀察的",
"observation","n.觀察",
"observatory","n.天文臺;氣象臺",
"observe","v.觀察",
"obsession","n.成見;頑念",
"obstacle","n.障礙",
"obtain","v.得到",
"obvious","adj.明顯的",
"obviously","adv.明顯地",
"occasion","n.場合",
"occasional","adj.偶爾的",
"occupation","n.職業;占用",
"occupy","v.占領",
"occur","v.發生",
"ocean","n.海洋",
"o'clock","n.點鐘",
"October","n.十月",
"odd","adj.奇特的;奇數的",
"odds","n.可能的機會",
"offend","v.冒犯;觸怒",
"offense","n.罪過;冒犯",
"offensive","adj.使人不愉快",
"offer","n/v.提議;提供",
"office","n.辦公室",
"officer","n.軍官;官員",
"official","adj./n.官方的;官員",
"offspring","n.後代;後裔",
"often","adv.時常",
"oil","n.油;石油",
"ointment","n.藥膏",
"old","adj.老的;舊的",
"omen","n.微兆;預兆",
"ominous","adj.惡兆的;不祥的",
"once","prep./adv./conj./n. 曾經;一旦",
"one","adj./n.一",
"onset","n.進攻;有力的開始",
"operational","adj.操作的;管理的",
"opponent","n.對手",
"oppress","v.壓迫;壓制",
"optical","adj.視覺的",
"optimism","n.樂觀;樂天",
"optimistic","adj.樂觀的;有信心的",
"option","n.選擇權;選擇力",
"optional","adj.可選擇的;隨意的",
"ordering","n.安排;佈置",
"ordinance","n.法令;條例",
"ordinarily","adj.正常地;普通地",
"organization","n.組織;團體",
"organize","v.組織",
"orient","v.使朝向東方",
"oriental","n./adj.東方人;東方的",
"origin","n.起源;由來",
"original","adj./n.最初的;原著",
"originality","n.創新;創造力",
"ornament","n.裝飾",
"orphan","n.孤兒",
"orphanage","n.孤兒院",
"orthodox","adj.正統的;傳統的",
"other","adj./n./prop. 另一個的;另一方",
"otherwise","adv.除此之外",
"oust","v.驅逐",
"out","prep/adv.在外",
"outbreak","v.爆發;發生",
"outcome","n.結果",
"outdoor","adj.戶外的",
"outdoors","adv.在戶外",
"outfit","n.裝備;用具",
"outing","n.遠足",
"outlaw","n.罪犯",
"outlet","n.出口;出路",
"outline","n./v.輪廓;綱要",
"outnumber","v.比·多",
"out-of-date","adj.過時的",
"outrage","n.暴行",
"outrageous","adj.駭人的;暴虐的",
"outset","n.開始",
"outside","prep./adj./adv./n. 在外面;外部的",
"outskirts","n.邊界;市郊",
"outstanding","adj.傑出的",
"outward","adj.外面的;肉體的",
"oven","n.烤箱",
"over","prep./adv.在...之上",
"overcoat","n.外套大衣",
"overcome","v.克服",
"overdo","v.做太多或過份",
"overeat","v.暴食",
"overhear","v.偷聽;無意中聽到",
"overlap","v.部分重疊;部分相同",
"overlook","v.俯視",
"overnight","ad v.在昨夜",
"overseas","adj./ad v.來自海外的",
"overtake","v.追上",
"overthrow","v.打翻",
"overturn","v.使傾覆;推翻",
"overwhelm","v.壓倒;淹沒",
"overwork","v.操勞過度",
"owe","v.虧欠",
"owl","n.貓頭鷹",
"own","adj./v.自己的",
"owner","n.持有者",
"ownership","n.所有權",
"ox","n.公牛",
"oxidation","n.氧化(作用)",
"oxygen","n.氧",
"oyster","n.牡蠣",
"pace","n./v.步;踱步",
"pack","n./v.包裹;捆",
"package","n./v.包;包裝",
"pact","n.協定;公約",
"page","n.頁",
"pain","n.疼痛",
"painful","adj.痛苦的",
"painfully","ad v.痛苦地",
"paint","n./v.油漆;顏料",
"painter","n.畫家",
"painting","n.圖畫",
"pair","n.一對",
"pajamas","n.睡衣褲",
"pal","n.夥伴",
"palace","n.宮殿",
"pale","adj.臉色蒼白的",
"palm","n.棕櫚;椰子",
"pan","n.平底鍋",
"panacea","n.萬靈藥",
"pancake","n.煎餅",
"panda","n.熊貓",
"pane","n.窗玻璃片",
"panel","n.板",
"panic","n./v.恐慌",
"panorama","n.全景",
"pants","n.褲子",
"papa","n.爸爸(非正式)",
"papaya","n.木瓜",
"paper","n.紙",
"parachute","n.降落傘",
"parade","n.遊行",
"paradise","n.天堂",
"paradoxically","adv.似是而非地",
"paragon","n.模範",
"paragraph","n.段落",
"paralyze","v.使麻痺;使癱瘓",
"paramount","adj./n.至上的;最高統治者",
"pardon","n./v.原諒",
"parent","n.父母",
"park","n.公園",
"park","v.停車",
"parking","n.停車",
"parlor","n.起居室;會客室",
"parrot","n.鸚鵡",
"part","n.一部分",
"partial","adj.局部的",
"partially","adv.部分地",
"participant","n.參與者;分享者",
"participate","v.參與",
"particular","adj.獨特的",
"partner","n.伙伴",
"partnership","n.合夥",
"part-time","adj./adv.兼任的",
"party","n.派對",
"pass","n./v.通行許可;經過",
"passage","n.通行",
"passenger","n.乘客",
"passerby","n.穿過;通過",
"passion","n.熱情",
"passionate","adj.熱情的",
"passive","adj.被動的",
"passport","n.護照",
"past","adj./prep.過去的",
"past","n.過去",
"paste","n.麵團",
"pasture","n.牧地;草原",
"pat","n.拍",
"path","n.小徑",
"pathetic","adj.悲哀的",
"patience","n.耐心",
"patient","adj.有耐心的",
"patient","n.病人",
"patiently","adv.耐心地",
"patriot","n.愛國者",
"patriotic","adj.愛國的",
"patron","n.贊助人",
"pattern","n.樣式",
"pause","n./v.暫停",
"pave(ment)","v.(n.)鋪(用於被動)",
"pavement","n.人行道",
"paw","n.腳掌;腳爪",
"pawn","v.典當",
"pawnshop","n.當鋪",
"pay(ment)","n/v.付錢",
"payday","n.發薪日",
"pea","n.豌豆",
"peace","n.和平",
"peaceful","adj.和平的",
"peach","n.桃子",
"peak","n.山頂",
"peanut","n.花生",
"pear","n.梨",
"pearl","n.珍珠",
"peasant","n.佃農",
"peculiar","adj.奇怪的",
"pedal","n.踏板",
"pedestrian","n.行人",
"peel","n./v.果皮;剝皮",
"peep","v./n.窺視;偷看",
"peer","n.同輩",
"pen","n.筆",
"penalty","n.處罰",
"pencil","n.鉛筆",
"penetrate","v.進入;看穿",
"penetrating","adj.敏銳的",
"penguin","n.企鵝",
"penicillin","n.盤尼西林",
"penis","n.男生殖器",
"penny","n.一分錢",
"pension","n.養老金",
"pentagon","n.五角形",
"people","n.人們",
"pepper","n.胡椒粉",
"per","prep.每",
"percent","n.百分之...",
"percentage","n.百分比",
"perch","n.棲木;高而安全的地位",
"perfect","adj.完美的",
"perfection","n.完備",
"perform","v.執行;演出",
"performance","n.成就;表演",
"performer","n.執行者;表演者",
"perfume","n.香味;香水",
"perhaps","adv.也許",
"peril","n.危險",
"period","n.一段時問",
"periodic","adj.定期的",
"periodical","n.期刊",
"perish","v.毀滅;死亡",
"permanent","adj.長久的",
"permission","n.准許",
"permit","n./v.許可",
"perpetual","adj.永久的",
"perplex","v.使困惑",
"perseverance","n.不屈不撓",
"persist","v.堅持",
"persistent","adj.堅持的",
"person","n.人",
"personal","adj.個人的",
"personality","n.個性",
"perspective","n.透視(畫)法",
"persuade","v.說服",
"persuasion","n.說服",
"persuasive","adj.有說服力的",
"pertain","v.有關",
"pessimism","n.悲觀(主義)",
"pest","n.害蟲",
"pesticide","n.殺蟲劑",
"pet","n./v.寵物;愛撫",
"petition","n.請願(書)",
"petroleum","n.石油",
"petty","adj.不重要的",
"pharmacy","n.藥房",
"phenomenon","n.現象",
"philharmonic","adj.愛樂(團體)的",
"philosopher","n.哲學家",
"philosophical","adj.哲學的",
"philosophy","n.哲學",
"phone","n./v.電話",
"photograph/photo","n.照片",
"photographer","n.攝影者",
"photographic","adj.攝影(用)的",
"photography","n.攝影術",
"phrase","n./v.片語;用語言表示",
"physical","adj.身體的",
"physicist","n.物理學家",
"physics","n.物理學",
"physiological","adj.生理學的",
"pianist","n.鋼琴家",
"piano","n.鋼琴",
"pick","n./v.十字鎬;挑選",
"picket","n.糾察隊員",
"pick-up","n.擴音器",
"picnic","n./v.野餐;去郊遊野餐",
"picture","n./v.圖畫;想像",
"picturesque","adj.似畫的",
"pie","n.派",
"piece","n./v.破片;拼合",
"piety","n.虔誠",
"pig","n.豬",
"pigeon","n.鵲子",
"pigment","n.顏料",
"pile","n./v.堆;疊",
"pilgrim","n.朝聖者",
"pilgrimage","n.朝聖者的旅程",
"pill","n.藥丸",
"pillar","n.柱子",
"pillow","n.枕頭",
"pilot","n./v.飛行員;駕駛",
"pimple","n.粉刺;面皰",
"pine","n.松樹",
"pineapple","n.鳳梨",
"ping-pong","n.乒乓球",
"pink","adj./n.粉紅色的",
"pint","n.品脫(液量名)",
"pioneer","n./v.拓荒者;開拓",
"pious","adj.虔誠的",
"pipe","n./v.管;用管道輸送",
"pipeline","n.管線",
"pirate","n.海盜",
"piston","n.活塞",
"pit","n.坑;煤礦",
"pitch","n.瀝青",
"pitcher","n.大水罐;投手",
"pity","n.可惜",
"pizza","n.比薩",
"place","n./v.地方;放置",
"plague","n./v.瘟疫;折磨",
"plain","adj.簡單的",
"plainly","adv.清楚地",
"plan","n./v.計劃",
"plane","n.飛機",
"planet","n.行星",
"planetary","adj.行星的",
"planning","n.計畫",
"plant","n./v.植物;種植",
"plantation","n.種植園",
"plaster","n.灰泥",
"plastic","adj./n.塑膠",
"plate","n.盤;碟",
"platform","n.月臺",
"Plato","n.柏拉圖(古希臘哲學家)",
"play","n./v.玩耍;玩",
"playground","n.操場",
"playmate","n.玩伴",
"playwright","n.劇作家",
"plea","n.懇求",
"pleasant","adj.令人愉快的",
"please","v.(使)高興",
"pleased","adj.高興的",
"pleasure","n.愉快",
"pledge","v.發誓;保證",
"plentiful","adj.豐富",
"plenty","n.大量",
"plot","n./v.情節;圖謀",
"plow","v.耕(地)",
"plug","n.塞子",
"plumber","n.管子工",
"plus","prep./n./adj.加",
"pneumonia","n.肺炎",
"pocket","n.口袋",
"poem","n.詩",
"poet","n.詩人",
"poetry","n.詩",
"point","n./v.尖端;持向",
"poison","n./v.毒藥",
"poisonous","adj.有毒的",
"pole","n.南極或北極",
"police","n.警察",
"policeman","n.警察",
"policy","n.政策",
"Polish","adj.波蘭的;波蘭人(語)的",
"polish","v.擦亮",
"polite","adj.有禮貌的",
"political","adj.政府的",
"politician","n.政治家",
"politics","n.政治活動",
"poll","n.民意調查",
"pollutant","n.污染物",
"pollute","v.污染",
"pollution","n.污染",
"polynomial","n.多詞學名;多項式",
"pond","n.池塘",
"ponder","v.深思",
"pony","n.小馬",
"pool","n.水塘",
"poor","adj.貧窮的",
"pop","n./v.砰然(聲)",
"popcorn","n.玉米花",
"popular","adj.流行的",
"popularity","n.流行",
"populate","v.移居於",
"population","n.人口",
"pork","n.豬肉",
"port","n.港",
"portable","adj.便攜的",
"porter","n.搬行李工人",
"portfolio","n.文件夾;投資組合",
"portion","n.一部分",
"portrait","n.肖畫像",
"portray","v.描繪",
"pose","n./v.姿勢",
"position","n.位置",
"positive","adj.確信的;積極的",
"possess","v.擁有",
"possibility","n.可能性",
"possible","adj.可能的",
"post","n.位置;郵局",
"post","v.貼出(佈告 v.通告)",
"postage","n.郵資",
"poster","n.海報",
"postman","n.郵差",
"postpone","v.延期",
"posture","n.姿勢",
"post-war","adj.戰後的",
"pot","n.罐;壺;盒",
"potato","n.馬鈴薯",
"potent","adj.有效力的",
"potential","adj./n.潛在的;僭能",
"potentiality","n.潛在性",
"potter","n./v.陶藝家;閒逛",
"pottery","n 陶器",
"poultry","n.家禽(肉)",
"pound","n.磅",
"pound","v.搗碎",
"pour","v.灌 v.注 v.倒",
"poverty","n.貧窮",
"powder","n.粉末",
"power","n.權力",
"powerful","adj.強健的",
"practical","adj.實踐的",
"practice","n./v.練習;實踐",
"practitioner","n.開業者",
"praise","n./v.讚美;稱讚",
"praiseworthy","adj.值得讚揚的",
"pray","v.祈禱",
"prayer","n.祈禱",
"preacher","n.傳教士",
"precaution","n.預防措施",
"precedent","n.慣例",
"precious","adj.珍貴的",
"precise","adj.精確的",
"predecessor","n.前輩",
"predicament","n.困境",
"predict","v.預言",
"prediction","n.預言",
"preface","n.前言",
"prefer","v.寧願選擇",
"preferable","adj.更適合的",
"preferably","adv.更好地",
"preference","n.偏愛",
"pregnant","adj.懷孕的",
"prejudice","n.成見",
"preliminary","adj./n.初步的;初步行動",
"premature","adj.過早的",
"premier","n.首相",
"premise","n.前提",
"premium","n.保險費",
"preoccupation","n.全神貫注",
"preoccupy","v.使全神貫注",
"preparation","n.準備",
"prepare","v.準備",
"prescribe","v.開(樂方)",
"prescription","n.藥方",
"presence","n.出席",
"present","(1) adj.出席的",
"present","(2) n 禮品",
"present","(3) v.贈送",
"presentation","n.表演",
"presently","adv.不久",
"preservation","n.保持",
"preserve","v.保存",
"preside","v.主持",
"presidency","n.總統(校長)的職位",
"president","n.總統;校長",
"presidential","adj.總統(校長)的",
"press","v.按",
"pressure","n.壓;按",
"presumption","n.推測",
"pretend","v.假裝",
"pretty","(1) adj.漂亮的",
"pretty","(2) adv.頗;相當",
"prevent","v.預防",
"preventive","adj.預防的",
"preview","n.預習;預演",
"previous","adj.以前的",
"price","n./v.價格",
"pride","n./v.自豪",
"primary","adj.主要的",
"prime","adj.首先的",
"primitive","adj.遠古的",
"prince","n.王子",
"princess","n.公主",
"principal","(1) adj.主要的",
"principal","(2) n.校長",
"principally","adv.主要(地)",
"principle","n.原則",
"print","v./n.印刷;字體",
"printer","n.印刷業從業人員",
"priority","n.居先;優先(權)",
"prison","n.監獄",
"privacy","n.獨處(狀況)",
"private","adj.個人的",
"privately","adv.私下地",
"privilege","n.特權",
"prize","n./v.獎品;珍視",
"probable","adj.很可能的",
"probe","v.探查",
"problem","n.問題",
"procedure","n.程序",
"proceed","v.繼續進行",
"proceeding","n.措施",
"process","n./v.過程;加工",
"processing","n.過程",
"proclamation","n.公告",
"produce","v.生產",
"product","n.產品",
"production","n.生產;產量;產品",
"productive","adj.多產的",
"productivity","n.生產力",
"profess","v.自稱",
"profession","n.職業",
"professional","adj./n.專業的",
"professor","n.教授",
"proficient","adj.精通的",
"profile","n.側面像;輪廓",
"profit","n./v.利益;有益於",
"profitable","adj.有益的",
"program","n./v.程序;編製程序",
"progress","n./v.前進",
"progressive","adj.不斷前進的",
"prohibit","v.禁止",
"prohibition","n.禁止",
"project","(1) n.項目",
"project","(2) v.投擲;凸出",
"projection","n.凸出物;設計;投影",
"prominent","adj.突起的",
"promise","n./v.諾言;許諾",
"promising","adj.大有希望的",
"promote","v.提升",
"promotion","n.晉級",
"prompt","adj.促使",
"prone","adj.易於發生(不愉快事)的;有傾向的",
"pronoun","n.代名詞",
"pronounce","v.發音",
"proof","n.證據",
"proper","adj.適合的",
"property","n.財產",
"prophesy","v.預言",
"proposal","n.計劃;建議",
"propose","v.提議",
"prose","n.散文",
"prosecute","對...起訴",
"prosecution","n.(被)起訴",
"prospect","n.期望",
"prosper","v.興旺",
"prosperity","n.發達",
"prosperous","adj.繁榮的",
"prostitute","n./v.妓女;出賣(名譽等)",
"protect","v.保護",
"protective","adj.保護的",
"protein","n.蛋白質",
"protest","n./v.抗議",
"proud","adj.白尊的",
"proudly","adv.驕傲地",
"prove","v.證明",
"proverb","n.諺語",
"provide","v.提供",
"provincial","adj.省的",
"provoke","v.激怒",
"prudent","adj.小心謹慎的",
"prune","v.修剪;刪除",
"psalm","n.聖歌",
"psychiatrist","n.精神科醫生",
"psychiatry","n.精神病學",
"psychological","adj.心理(上)的",
"psychologist","n.心理學家",
"psychology","n.心理學",
"pub","n.小酒館",
"public","adj./n.公眾的;大眾",
"publication","n.公布",
"publicity","n.眾人皆知",
"publish","v.出版",
"publisher","n.出版社(者)",
"pull","v.拖;拉",
"pulmonary","adj.肺(病)的",
"pulse","n.脈搏",
"pump","n.泵",
"pumpkin","n.南瓜",
"punctual","adj.如期的;準時的",
"punctuation","n.標點(法)",
"punish(ment)","v.(n)處罰",
"pupil","n.小學生",
"PUPPY","n.小狗",
"purchase","n./v.購買",
"pure","adj.純淨的",
"purely","adv.完全地",
"purity","n.純潔",
"purple","adj./n.紫色的",
"purpose","n.意圖",
"purse","n.錢包",
"pursue","v.追捕",
"pursuit","n.追趕",
"push","推動",
"put","v.安置",
"puzzle","n./v.難題;困惑",
"pyjamas","n.睡衣褲",
"quaint","adj.奇特的",
"quake","v.震動",
"qualify","v.使具有資格",
"quality","n.質量",
"quantity","n.數量",
"quarrel","n./v.爭吵",
"quarrelsome","adj.好爭吵的",
"quarter","n.四分之一",
"queen","n.王后",
"queer","adj.奇怪的",
"quest","n.探索",
"question","n./v.問題;詢問",
"questionnaire","n.問卷",
"queue","n./v.行列;排隊",
"quick","adj./adv.快速的/地",
"quiet","adj./v.安靜的;使安靜",
"quit","v.停止",
"quite","adv.完全;非常",
"quiz","n.問答比賽",
"quotation","n.引語",
"quote","v.引用",
"rabbit","n.兔",
"race","n.比賽",
"racial","adj.種族的",
"rack","n.置物架",
"radar","n.雷達",
"radiate","v.發射",
"radio","n.收音機",
"rag","n.破布",
"rage","n./v.發怒",
"raid","n.突擊",
"railroad","n.鐵路",
"railway","n.鐵路",
"rain","n./v.雨",
"rainbow","n.彩虹",
"raincoat","n.雨衣",
"rainfall","n.降雨",
"rainy","adj.多雨的",
"raise","v./n.舉起;上漲",
"rally","v./n.集合;開玩笑",
"rancher","n.農場主人",
"range","n.區域",
"rank","n./v.等級;並排",
"ransom","n.贖金",
"rape","v.強姦",
"rapid","adj.快的",
"rare","adj.珍貴的",
"rat","n.鼠",
"rate","n./v.比率;評估",
"rather","adv.與其不如",
"ratify","v.批准",
"rational","adj.理性的",
"raw","adj.生的",
"ray","n.光線",
"razor","n.舌」鬍刀",
"reach","n./v.抵達",
"react","v.反應",
"reaction","n.反應",
"reactionary","adj.保守的;反動的",
"read","v.讀",
"readiness","n.準備就緒;迅速",
"ready","adj.有準備的",
"real","adj.真的",
"realistic","adj.理論的",
"reality","n.真實性",
"realize","v.了解",
"reason","n.理由",
"reasonable","adj.有理由的;合理的",
"reassure","v.使安心",
"rebel","n.反抗者",
"rebel","v.謀反",
"rebellion","n 造反",
"rebellious","adj.難管教的",
"rebuild","v.重建",
"recall","v.想出",
"recapture","v.再體驗",
"receipt","n.收到;收據",
"receive","v.收到",
"receiver","n.接受者;受話器",
"recent","adj.最近的",
"reception","n.接收",
"recession","n.經濟衰退",
"recipient","n.接受者",
"reckless","adj.輕率的",
"reckon","v.推斷",
"recognition","n.承認",
"recognize","v.承認",
"recollect","v.想起",
"reconcile","v.調解",
"reconstruction","n.重建(物)",
"record","(1) n.記錄",
"record","(2) v.記錄",
"recorder","n.記錄者",
"recover","v.恢復",
"recovery","n.復原",
"recruit","v.招募",
"recycle","v.使再循環;使回收使用",
"red","adj./n.紅色的",
"reduce","v.減少",
"redundant","adj.過剩的",
"reel","n./v.捲軸;捲;蹣跚",
"refer","v.起因於...",
"reference","n.委託",
"referendum","n.公民投票",
"refine","v.提煉",
"refinement","n.精製品",
"reflect","v.反射",
"reflection","n.反映",
"reform","v.改革",
"refrain","v.抑制",
"refresh","v.使給予活力",
"refreshment","n.精神恢復",
"refrigerator","n.冰箱",
"refugee","n.難民",
"refund","n.退款",
"refusal","n.拒絕",
"refuse","v.拒絕",
"regain","v.取回",
"regard","n./v.注目;注意",
"regarding","prep.關於...",
"regiment","n.(軍)團",
"region","n.地區",
"regional","adj.地區的",
"register","n./v.記錄;登記",
"registration","n.記錄",
"regret","n./v.哀傷;遺憾",
"regular","adj.整齊的;規律的",
"regulate","v.限制;控管",
"regulation","n.管理;限制",
"rehabilitate","v.使恢復",
"rehabilitation","n.康復",
"rehearsal","n.排練;複述",
"rehearse","v.詳述;排演",
"reign","n.君主統治時期",
"rein","n.韁繩;統治",
"reject","v.拒絕;否認",
"rejoice","v.高興",
"relate","v.敘述",
"relationship","n.(親戚)關係",
"relative","(1) adj.比較上的",
"relative","(2) n.親戚",
"relatively","adv.相對地",
"relax","v.放鬆",
"release","n./v.解放",
"reliable","adj.可信賴的",
"relief","n.解除",
"relieve","v.減輕(痛苦)",
"religion","n.宗教",
"religious","adj.狂熱信仰的",
"relinquish","v.放棄",
"relish","n./v.美味;喜愛",
"reluctant","adj.不情願的;勉強的",
"reluctantly","adv.不願意地",
"rely","v.信賴",
"remain","v.殘留",
"remains","n.遺跡",
"remark","n./v.論述;評論",
"remarkable","adj.該注意的",
"remedy","n.治療",
"remember","v.記得",
"remind","v.使想起",
"reminiscence","n.回憶",
"reminiscent","adj.使人憶起...的",
"remote","adj.遠離的",
"remove","v.除去",
"renaissance","n.復興",
"renew","v.回復",
"renowned","adj.著名的",
"rent","n./v.地租;承租",
"reorganization","改組",
"repair","n./v.修理",
"repay","v.償還",
"repeat","v.重覆地說(做)",
"repel","v.擊退",
"repent","v.後悔",
"repetition","n.重覆",
"replacement)","v.(n.)放回",
"replaceable","adj.可取代的",
"reply","n./v.答覆",
"report","n./v.報告;述說",
"reporter","n.報告者",
"represent","v.描寫",
"representation","n.象徵",
"representative","adj./n.象徵的;例子",
"repress","v.抑制",
"republic","n.共和國",
"republican","adj./n.共和國的;共和主義者",
"reputation","n.名聲",
"request","n./v.要求",
"require(ment)","v.(n)需求",
"rescue","n./v.救助",
"research","n./v.研究",
"researcher","n.研究員",
"resemblance","n.相似",
"resemble","v.和...相似",
"resent","v.不滿",
"reservation","n.保留",
"reserve","v.預備",
"reservoir","n.蓄水池",
"reside","v.居住",
"resign","v.辭職",
"resignation","n.辭職;辭呈",
"resist","v.反抗",
"resistance","n.反抗",
"resolution","n.決心",
"resolve","v.決心;決定",
"resonance","n.共嗚",
"resort","v.前往",
"resort","n.度假地",
"resource","n.財產;資產",
"respect","n./v.尊敬",
"respectable","adj.可敬的",
"respectful","adj.尊重的",
"respective","adj.個別的",
"respond","v.回答",
"response","n.回答",
"responsibility","n.責任",
"responsible","adj.負責的",
"rest","n./v.休息",
"restaurant","n.餐廳",
"restoration","n.恢復",
"restore","v.歸還",
"restraint","n.克制",
"restrict","v.限制",
"restroom","n.廁所",
"result","n./v.結果;起因於...”",
"resultant","adj.做為...後果的",
"resume","v.重新開始",
"retail","n./adj./adv.零售;零售的(地)",
"retaliate","v.報復",
"retirement)","v.(n)退休",
"retort","v./n.反駁",
"retreat","v.後退;撒退",
"retrieve","v.找回",
"return","n./v.回復;歸還",
"reunion","n.重新結合;聚會",
"reveal","v.揭露",
"revenge","n./v.報復",
"reverse","adj./n./v.相反的;反轉",
"review","v./n.再檢查;複習",
"revise","v.校正",
"revision","n.修正",
"revive","v.復活",
"revolution","n.革新",
"revolutionary","adj.革命(性)的",
"revolve","v.使旋轉",
"reward","n./v.報酬",
"rhetoric","n.修辭學",
"rhythm","n.節奏",
"rhythmic","adj.韻律的",
"rib","n.肋骨",
"ribbon","n.絲帶",
"rice","n.米",
"rich","adj.富有的",
"rid","n./v.解除;免談",
"riddle","n.謎題",
"ride","n./v.騎;騎乘",
"ridiculous","adj.可笑的",
"right","adj./adv./n.正確的",
"righteous","adj.正直的",
"rigorous","adj.嚴密的",
"ring","n./v.圈;圍住",
"riot","n.暴動",
"ripe","adj.成熟的",
"rise","n./v.上升;站起來",
"risk","n./v.危險",
"rite","n.儀式",
"ritual","n.(儀式的)活動",
"rival","n.對手",
"river","n.河流",
"roach","n.蟑鄉;大麻煙卷的煙蒂",
"road","n.路",
"roar","n./v.怒吼",
"roast","n.吐司",
"rob","v.搶劫",
"robe","n.長袍",
"robin","n.知更鳥",
"robot","n.機器人",
"rock","n.岩石",
"rocket","n.火箭",
"role","n.角色",
"roll","n./v.滾動",
"romance","n.羅曼蒂克",
"romantic","adj./n.羅曼蒂克的",
"roof","n.屋頂",
"room","n.房問",
"rooster","n.公雞",
"root","n.根",
"rope","n.繩子",
"rose","n.玫瑰",
"rot","v./n.腐敗;腐朽",
"rotate〝使旋轉","",
"rotation","n.旋轉",
"rotten","adj.腐敗的",
"rough","adj.不平的;粗魯的",
"roughly","adv.大體上",
"round","adj./n.圓的",
"route","n.路線",
"routine","adj./n.慣常的",
"row","n./v.爭吵",
"royal","adj.王室的",
"royalty","n.皇室成員",
"rub","v.摩擦",
"rubber","n.橡膠",
"rude","adj.不禮貌的",
"rug","n.小地毯",
"rugged","adj 崎嶇的",
"ruin","n./v.毀滅",
"rule","n./v.規則",
"ruler","n.統治者",
"rumor","n.謠言",
"run","v.跑",
"runner","n.奔跑的人",
"rush","n./v.衝;急速前進",
"rust","n./v.銹;生銹",
"rusty","adj.的鐵釘",
"sack","n.大袋子",
"sad","adj.難過",
"saddle","n.馬鞍",
"safe","adj.安全的",
"safety","n.安全性",
"sail","n./v.船;坐船旅行",
"sailor","n.水手",
"sake","n.為二好處著想",
"salad","n.沙拉",
"salary","n.薪水",
"sale","n.出售;拍賣",
"salesman","n.銷售員",
"saloon","n.大轎車;酒館;交誼廳",
"salt","n.鹽",
"salute","v.行舉手禮",
"salvage","n./v.搶救",
"salvation","n.救星",
"same","adj./pron.同樣的",
"sample","n.樣品",
"sanction","n.批准",
"sand","n.沙",
"sandal","n.涼鞋",
"sandwich","n.三明治",
"sanitation","n.衛生設備",
"sarcastic","adj.諷刺的",
"Satan","n.魔鬼;撒旦",
"satellite","n.衛星",
"satire","n.諷刺文學",
"satisfaction","n.滿意",
"satisfactorily","ad 二令人滿意地",
"satisfactory","adj.滿意的",
"satisfy","v.滿意",
"Saturday","n.星期六",
"sauce","n.醬料",
"sausage","n.香腸",
"savage","adj.兇猛的",
"save","v.拯救",
"saving(s)","n.存款",
"saw","n.鋸",
"say","v.說",
"scale(s)","n.標準",
"scan","v.細看;掃描",
"scandal","n.醜聞",
"scare","v.恐嚇",
"scared","adj.害怕的",
"scarf","n.領巾",
"scarlet","adj.鮮紅色的",
"scary","adj.可怕的",
"scatter","v.散開",
"scene","n.場景",
"scenery","n.風景",
"schedule","n./v.時間表",
"scholar","n.學者",
"scholastic","adj.學校的",
"school","n.學校",
"science","n.科學",
"scientific","adj.科學的",
"scientist","n.科學家",
"scissor(s)","n.剪刀",
"scold","v.責備",
"score","n./v.刻痕;作訊號",
"scorn","n./v.輕蔑;鄙視",
"scout","n.偵察機",
"scrap","n.碎片",
"scrape","v.把...括除",
"scratch","v.抓(傷)",
"scream","n./v.尖叫聲;尖叫",
"screen","n.屏幕",
"script","n.原稿",
"scrutiny","n.細查",
"sculpture","n.雕塑(品)",
"sea","n.海洋",
"seal","(1) n.海豹;印章",
"seal","(2) v.密封",
"seam","n.接縫",
"search","v./n.搜尋",
"season","n.季節",
"seat","n./v.座位;坐下",
"Second","adj./n.第 v.的;秒",
"secondary","adj.次要的",
"secret","adj./n.祕密的",
"secretary","n.秘書",
"section","n.地域",
"secular","adj.世俗的",
"secure","adj.安全的",
"security","n.安全",
"seduce","v.慫恿;引誘",
"seek","v.尋覓",
"seem","v.似乎",
"seemingly","adv.看樣子",
"segregate","v.隔離開",
"seize","v.扣押",
"seldom","adv.很少;不常",
"select","v.挑選",
"selection","n.選擇",
"self","pron.本身",
"selfish","adj.自私的",
"sell","v.賣",
"semester","n.一學期",
"seminar","n.研討會",
"send","v.送",
"sensation","n.感覺",
"sense","n./v.感官;覺得",
"sensibility","n.判斷力",
"sensible","adj.明智的",
"sensitive","adj.敏感的",
"sentence","n.判決",
"sentimental","adj.感情上的",
"separate","adj./v.分開的",
"separately","adv.個別地",
"separation","n.分開",
"September","n.九月",
"sequential","adj.隨之而來的",
"serious","adj.嚴肅的",
"sermon","n.說教;佈道",
"serum","n.血清",
"servant","n.僕人",
"serve","v.侍候",
"service","n.服侍",
"set","n./v.放置",
"settle(ment)","v.(n)決定",
"settler","n.移民",
"seven","adj./n.七",
"seventeen","adj./n.十七",
"seventy","adj./n.七十",
"sever","v.割斷",
"several","adj./n.幾個的;幾個",
"severe","adj.嚴格的",
"severely","adv.嚴重地",
"sew","v.縫",
"sewer","n.下水道",
"sex","n.性(別)",
"sexual","5adj.男女之問的",
"shade","n.陰蔭",
"shadow","n.陰影",
"shady","adj.遮蔭的",
"shaft","n.把手",
"shake","n./v.搖動",
"shallow","adj.淺的",
"shame","n./v.恥辱",
"shameful","adj.可恥的",
"shampoo","n.洗髮(精)",
"shape","n./v.形狀;做成",
"share","n./v.分擔;分配",
"sharp","adj.尖銳的",
"shave","n./v.刮鬍子;剃",
"shear","n.剪;切斷",
"shed","v.使流出",
"sheep","n.羊",
"sheep","n.沒有主見的人",
"sheer","adj.純粹的",
"shepherd","n./v.牧羊人;引導",
"shelf","n.架子",
"shell","n.外觀;外表",
"shelter","n./v.避難所",
"shepherd","n.牧羊人",
"shift","n.變換",
"shine","n./v.光亮",
"shiny","adj.閃爍的",
"ship","n.船",
"shipment","n.一批貨物",
"shirt","n.襯衫",
"shiver","v.顫抖",
"shock","n./v.震驚",
"shocked","adj.使震驚",
"shocking","adj.受震驚的",
"shoe","n.鞋",
"shoot","v.發射",
"shop","(1) n 商店",
"shop","(2) v.購物",
"shore","n.岸;濱",
"short","adj.短的",
"shortage","n.短缺",
"shortcoming","n.缺點",
"shorten","v.使縮短",
"shorts","n. [pl.]短褲",
"shortsighted","adj.近視的",
"shot","n.射擊",
"should","aux.應該",
"shoulder","n./v.肩膀;肩負",
"shout","n./v.喊叫(聲)",
"show","n./v.表演;展現",
"shower","n.陣雨",
"shrimp","n.小蝦;矮子",
"shrink","v.縮小",
"shut","v.關上",
"shy","adj.羞怯的",
"sibling","n.手足",
"sick","adj.有病的",
"side","n.",
"sidewalk","n.人行道",
"sigh","n./v.嘆息",
"sight","n./v.視力;看見",
"sightseeing","n.觀光",
"sign","n./v.記號;簽字",
"signal","n./v.信號",
"signature","n.簽字",
"significance","n.重要性",
"significant","adj.重要的",
"silence","n./v.寂靜",
"silent","adj.寂靜的",
"silently","adv.寂靜地",
"silk","n.絲",
"silly","adj.愚蠢的",
"silver","adj./n.銀製的;銀",
"similar","adj.相似的",
"similarly","adv.同樣地",
"similarity","n.相似之處",
"simple","adj.簡單的",
"simplicity","n.樸素",
"simplify","v.使單純",
"simultaneously","adv.同時",
"sin","n./v.罪惡",
"since","adv./prep.從...以來",
"sincere","adj.真心誠意的",
"sincerity","n.真誠",
"sing","v.歌唱",
"singer","n.歌手",
"single","adj.單一的",
"singular","adj.單數的",
"sinister","adj.陰險的",
"sink","(1)n.洗滌槽",
"sink","(2) v.下沉",
"sir","n.先生",
"sister","n.姊妹",
"sit","v.坐",
"site","n.場所",
"sitter","n.模特兒",
"situate","v.使位於",
"situated","adj.坐落在...的",
"situation","n.形勢",
"six","adj./n.六",
"sixteen","adj./n.十六",
"sixty","adj./n.六十",
"sizable","adj.相當大的",
"size","n.尺寸",
"skate","n./v.冰鞋;溜冰",
"skeleton","n.骨骸",
"sketch","n./v.素描",
"skill","n.技巧",
"skillful","adj.熟練的",
"skin","n./v.皮膚;剝...的皮",
"skinny","adj.消瘦的",
"skip","n./v.輕跳;略過",
"skirt","n.裙子",
"sky","n.天空",
"slam","v.使勁關閉",
"slap","n.一巴掌",
"slash","v.亂砍",
"slave","n. 奴隸",
"sleep","n./v.睡眠",
"sleepy","adj.想睡的",
"sleeve","n.衣服袖子",
"slender","adj.修長的;苗條的",
"slice","n.片",
"slide","n./v.打滑",
"slight","adj.輕微的",
"slim","adj.苗條的",
"slip","n./v.滑倒",
"slipper","n.拖鞋",
"slippery","adj.滑溜的",
"slogan","n.口號",
"slope","n.斜坡",
"sloppy","adj.馬虎的",
"slow","adj.緩慢的",
"slug","n.子彈;蛞蝓",
"slum","n.貧民窟",
"small","adj.小的",
"smallpox","n.天花",
"smart","adj.聰明的",
"smell","n./v.氣味;聞",
"smile","n./v.微笑",
"smog","n.煙霧",
"smoke","n./v.煙;抽煙",
"smooth","adj.光滑的",
"smuggle","v.走私",
"snack","n.點心",
"snail","n.牛",
"snake","n.蛇",
"snap","n./v.快照;突然折斷",
"snatch","v.搶",
"sneak","v.鬼鬼祟祟地走",
"sneaker","n.鬼鬼祟祟做事的人",
"sneeze","n./v.打噴嚏",
"snow","n./v.雪",
"snowy","adj.多雪的",
"so","adv./conj.所以",
"soak","v.浸泡",
"soap","n.肥皂",
"sob","v.啜泣",
"sober","adj.清醒的",
"soccer","n.足球",
"social","adj.社會的",
"socialism","n.社會主義",
"socially","adj.善於交際地",
"society","社會",
"sociological","adj.社會學的",
"sociology","n.社會學",
"sock(s)","n.短襪",
"soda","n.蘇打水",
"sodium","n.鈉",
"sofa","n.沙發",
"soil","n./v.土壤;弄髒",
"solar","adj.太陽的",
"soldier","n.軍人",
"sole","n.腳底(板);鞋底",
"solely","adj.唯一地",
"solicitor","n.懇求者;事務律師",
"solid","adj.堅固的",
"solidarity","n.團結",
"solitary","adj.獨居的",
"solitude","n.孤獨",
"solo","n./adj.獨唱;單獨的",
"solve","v.解答",
"some","adj.一些",
"somebody","pron.有人",
"someday","adv.有朝一日",
"somehow","adv.不知怎麼地",
"someone","pron.某人",
"something","pron.某物",
"sometime","adv.在將來某個時候",
"sometimes","adv.有時",
"somewhat","adv.有點",
"somewhere","adv.某處",
"son","n.兒子",
"song","n.歌曲",
"soon","adv.很快",
"sore","adj.疼痛的",
"sorrow","n.悲傷",
"sorry","adj.感到抱歉的;感到遺憾的",
"sort","n./v.種類;把·分類",
"soul","n.靈魂",
"sound","adj.無恙的",
"sound","n./v.聲音;聽起來",
"soup","n.湯",
"sour","adj.酸的",
"source","n.來源",
"south","adj./adv./n.南方的",
"southern","adj.南方的",
"souvenir","n.紀念品",
"sovereign","n.元首",
"sovereignty","n.主權",
"sow","v.播(種)",
"space","n.空間",
"spade","n.鏟",
"spaghetti","n.義大利麵條",
"spare","adj./v.多餘的;撥出",
"sparkle","n./v.閃耀",
"sparrow","n.麻雀",
"speak","v.說話",
"speaker","n.演講者",
"special","adj.特別的",
"species","n.種類",
"specific","adj.明確的",
"spectacles","n.眼鏡",
"spectacular","adj.壯觀的",
"spectra","n.光譜",
"speculate","v.推測;投機",
"speech","n.演講",
"speed","n/v.速度;急行",
"spell","v.拼寫",
"spelling","n.拼寫能力",
"spend","v.花費",
"sperm","n.精子",
"spice","n./v.香料;加香料於",
"spider","n.蜘蛛",
"spill","n./v.溢出;濺出",
"spinach","n.菠菜",
"spirit","n.精神;心靈",
"spiritual","adj.精神的;心靈的",
"spit","n./v.口水;吐",
"spite","n.惡意",
"splash","n./v.飛澱(聲)",
"splendid","adj.輝煌的",
"split","n./v.裂縫;(使)裂開",
"spoil","v.破壞",
"sponsor","v./n 贊助(者)",
"spontaneous","adj.自發的",
"spoon","n./v.匙;用匙舀",
"sport","n.運動",
"sportsman","n.運動員",
"spot","n./v.地點;散布",
"spouse","n.配偶",
"sprawl","v.伸開四肢躺(坐)",
"spray","v.噴;濺散",
"spread","n./v.擴展",
"spring","n.春天",
"spring","v.跳躍",
"spur","n.馬刺;激勵",
"spy","n./v. 間諜;秘密監視",
"squad","n.爭吵",
"square","adj./n.四方的",
"squarely","adv.正直地",
"squeeze","v.壓;擠",
"squirrel","n.松鼠",
"stab","n./v.刺;戳",
"stability","n.穩定性",
"stable","adj.穩定的",
"stack","v.使成堆",
"stadium","n.(露天)運動場",
"stage","n./v.舞臺;上演",
"stagger","v.蹣跚",
"stair","n.樓梯",
"stale","adj.不新鮮的",
"stall","n.攤位",
"stamp","n./v.郵票;跺腳",
"stand","n./v.攤位;站",
"standard","adj./n.標準",
"standardize","v.使標準化",
"standpoint","n.立場",
"staple","n./adj.釘書針;主要的",
"star","n./v.星;明星",
"stare","n./v.瞪眼",
"start","n./v.開始",
"startle","v.使大吃一驚",
"starve","v.(使)挨餓",
"state","n.狀態",
"state","v.陳述",
"statesman","n.政治家",
"static","adj.靜態的",
"station","n.車站",
"stationary","adj.靜止的",
"stationery","n.文具",
"statistical","adj.統計(學)的",
"statue","n.雕像",
"stature","n.高水平;身材",
"status","n.社會地位",
"statute","n.法令",
"stay","n./v.停留",
"steady","adj.穩固的",
"steak","n.牛排",
"steal","v.偷",
"steam","n./v.蒸汽",
"steel","n.鋼鐵",
"steep","adj.陡峭的",
"steer","v.駕駛",
"step","n./v.腳步;走",
"stepchild","n.妻與前夫(或夫與前妻)所生的孩子",
"stereo","n.立體聲;立體音響器材",
"stereotype","n.老套",
"steward","n.服務員",
"stick","v.刺入",
"sticky","adj.黏的",
"stiff","adj.硬的",
"still","adj./adv.靜止的;仍然",
"stimulation","n.刺激",
"stimulus","n.刺激(物)",
"sting","n./v.刺傷(處);刺",
"stingy","adj.小氣的",
"stir","v.攪動",
"stocking(s)","n.長襪",
"stomach","n.胃",
"stone","n./v.石頭",
"stool","n.凳子",
"stoop","v.彎腰",
"stop","n./v.車站;停止",
"storage","n.儲存",
"store","n./v.店鋪;儲存",
"storm","n./v.暴風雨;猛烈攻擊",
"stormy","adj.有暴風雨的",
"story","n.故事",
"stove","n.火爐",
"straight","adj./adv.筆直的",
"straighten","v.清理",
"strain","v.扭傷",
"strange","adj.奇怪的",
"stranger","n.陌生人",
"strategic","adj.戰略(性)的",
"strategy","n.策略",
"straw","n.稻草;麥桿",
"strawberry","n.草莓",
"streak","n./v.條紋;快速移動",
"stream","n./v.小河",
"street","n.街道",
"streetcar","n.(市內)有軌電車",
"strength","n.力氣",
"strengthen","v.加強",
"stress","n./v.壓力",
"stretch","n./v.伸展",
"strict","adj.嚴格的",
"stride","v.邁大步走",
"strike","n./v.罷工;猛然攻擊",
"string","n./v.細繩",
"strive","v.奮鬥",
"stroke","n.一擊",
"stroke","v.撫摸",
"strong","adj.強壯的",
"structural","adj.結構上的",
"structure","n.結構",
"struggle","n./v.掙扎;努力",
"stubborn","adj.固執的",
"student","n.學生",
"studio","n.工作室",
"study","n.學業",
"stu","勿 v.學習",
"stuff","n.材料",
"stuff","v.填滿",
"stupid","adj.愚蠢的",
"sturdy","adj.強健的",
"style","n.風格",
"subdue","v.使屈服",
"subject","n.主題",
"subjective","adj.主觀的",
"submarine","n.潛水艇",
"submit","v.提交",
"subscribe","v.訂購",
"subsequently","ad v.後來",
"subsidy","n.補貼",
"substance","n.物質",
"substantially","adv.充足地",
"subtract","v.減掉",
"suburb","n.郊區",
"succeed","v.達成",
"success","n.成功",
"successful","adj.成功的",
"successive","adj.連續的",
"such","adj./prop.如此的",
"suck","v.吸;吮",
"suckle","v.哺乳",
"sudden","adj.突然的",
"suffer","v.遭受",
"suffering","n.苦難",
"sufficient","adj.充足的",
"suffocate","v.(使)窒息而死",
"sugar","n.糖",
"sugarcane","n.甘蔗",
"suggest","v.建議",
"suggestion","n.提議",
"suicide","n.自殺",
"suit","n.一套衣服",
"suit","v.適合",
"suitable","adj.合適的",
"suitcase","n.手提箱",
"sum","n.總數",
"summarize","v.概述;總結",
"summary","n.摘要",
"summer","n.夏天",
"summit","n.山頂",
"summon","v.召喚",
"sun","n./v.太陽;曬太陽",
"Sunday","n.星期日",
"sunlight","n.陽光",
"sunny","adj.陽光充足的",
"sunset","n.日落",
"sunshine","n.陽光",
"super","adj.極好的",
"superb","adj.極好的",
"superficial","adj.表面(皮)的",
"superfluous","adj.過多的",
"superhighway","n.超級高速公路",
"superimpose","v.使重疊",
"superintendent","n.監護人;警長",
"superior","adj.上等的",
"superiority","n.優越",
"supermarket","n.超級市場",
"superstition","n.迷信",
"superstitious","adj.迷信的",
"supervise","v.監督",
"supervisor","n.監督人",
"supper","n.晚餐",
"supplement","n.補充",
"supply","n./v.供應",
"support","n./v.支持",
"suppose","v.猜想",
"supposedly","adv.據推測",
"suppress","v.鎮壓",
"sure","adj.確信的",
"surface","n.表面",
"surface","n./adi./v. 表面(的);顯露",
"surge","v.洶湧向前",
"surgeon","n.外科醫師",
"surgery","n.外科(手術)",
"surname","n.姓",
"surprise","n./v.驚訝;使驚奇",
"surprised","adj.感到驚奇的",
"surprising","adj.使人驚訝的",
"surrender","v.投降",
"surround","v.圍繞",
"surroundings","n.環境",
"survey","n./v.調查;審視",
"survival","n.殘存",
"survive","v.悻存",
"survivor","n.生還者",
"suspension","n.中止",
"suspicion","n.懷疑",
"suspicious","adj.猜疑的",
"swallow","n.燕子",
"swallow","v.吞下",
"swan","n.天鵝",
"sway","v.(使)搖動",
"swear","v.發誓",
"sweat","n./v.汗水;出汗",
"sweater","n.毛線衫",
"sweep","n. v.清掃;打掃",
"sweet","adj.甜的",
"sweet(s)n.糖果;甜點","",
"swift","adj.迅速的",
"swim","n. v.游泳",
"swing","n./v.鞦韆;搖擺",
"switch","n./ v.開關;轉變",
"sword","n.劍",
"syllable","n.音節",
"syllabus","n.教學大綱",
"symbol","n.象徵",
"symmetry","n.對稱(性)",
"sympathetic","adj.有同情心的",
"sympathize","v.同情",
"sympathy","n.同情心",
"symphony","n.交響樂",
"synonym","n.同義詞",
"synthesis","n.綜合(體);合成",
"system","n.系統",
"systematic","adj.有系統的",
"systematically","adv.有系統地",
"table","n.桌子",
"tablespoon","n.大湯匙",
"tablet","n.藥片",
"tabloid","n.小報",
"taboo","n.禁忌",
"tackle","v.處理",
"tag","n.標籤",
"tail","n.尾巴",
"tailor","n.裁縫師",
"take","v.拿(走)",
"tale","n.故事",
"talent","n.天賦",
"tally","n./v.聊天;說話",
"talkative","adj.健談的",
"tall","adj.高的",
"tame","adj.馴∣服的",
"tan","v.(使)曬黑",
"tangerine","n 橘子",
"tangible","adj.明確的;實質的",
"tank","n.大容器",
"tanker","n.油輪",
"tap","n./v.塞子;輕敲",
"tape","n./v.錄音帶",
"target","n./v.靶子;對·瞄準",
"tariff","n.關稅",
"task","n.任務",
"taste","n./v.味覺;品嚐",
"tasty","adj.美味的",
"tax","n./v.稅;對...微稅",
"taxi","n.計程車",
"tea","n.茶",
"teach","v.教導",
"teacher","n.教師",
"team","n.隊",
"tear","v.撕破 n.眼淚",
"tease","v.戲弄;取笑",
"technical","adj.技術性的",
"technician","n.技術人員",
"technique","n.技術",
"technological","adj.工業技術的;科技的",
"technology","n.工業技術;(一門)科技",
"teenager","n.青少年",
"telegram","n.電報",
"telegraph","n.電報",
"telephone","n.電話",
"telescope","n.望遠鏡",
"television","n.電視機",
"tell","v.告訴",
"teller","n.講故事者;銀行出納員",
"temper","n.脾氣",
"temperament","n.氣質",
"temperature","n.溫度",
"tempest","n.暴風(雨、雪)",
"temple","n.廟宇",
"temporary","adj.暫時的",
"tempt","v.慫恿",
"temptation","n.誘惑",
"ten","n.十",
"tenant","n.房客",
"tend","v.有...的傾向",
"tendency","n.癖好",
"tender","adj.嫩的",
"tennis","n.網球(運動)",
"tense","adj.緊張的",
"tension","n.緊張",
"tent","n 帳篷",
"tentative","adj.試驗性的",
"tern","n.期限",
"terminal","n.終點;總站",
"terminate","v.(使)終止",
"terrace","n.陽臺",
"terrible","adj.可怕的",
"terrific","adj.極好的;嚇人的",
"terrified","adj.嚇壞了的",
"terrify","v.使驚恐",
"territory","n.領土",
"test","n./v. 小考;考驗",
"testify","v.作證",
"text","n.本文",
"textbook","n.教科書",
"than","prep./conj.比;比較",
"thank","n./v.感謝",
"thankful","adj.感謝的",
"theater","n.戲院",
"theatrical","adj.戲劇的",
"theft","n.偷竊",
"theme","n.主題",
"then","adj./adv.那時",
"theologian","n.神學家",
"theological","adj.神學(上)的",
"theoretical","adj.理論(上)的",
"theory","n.理論",
"therapeutic","adj.有療效的",
"therapist","n.治療專家",
"there","n/adv.那裡",
"therefore","adv.因此",
"thermal","adj.熱(量)的",
"these","adj./pron.這些",
"thesis","n.論文",
"thick","adj.厚的",
"thief","n.小偷",
"thigh","n.大腿",
"thin","adj.薄的",
"thing","n.束西",
"think","v.想",
"third","adj./n.第三",
"thirst","n.口渴",
"thirsty","adj.口渴的",
"thirteen","adj./n.十三",
"thirty","adj./n.三十",
"thorough","adj.徹底的",
"those","adj./pron.那些",
"though","adv./conj.然而;雖然",
"thought","n.思想",
"thoughtful","adj.體貼的;沉思的",
"thousand","adj./n.一千",
"thread","n./v.線;穿線於",
"threat","n.威脅",
"threaten","v.恐嚇",
"three","adj./n.三",
"threshold","n.門檻;開端",
"thrift","n.節儉",
"thrill","v./n.使激動;興奮",
"thrilling","adj.令人激動的",
"thrive","v.興旺",
"throat","n.喉嚨",
"through","prep./adv.通過",
"throughout","prep./adv.遍及",
"throw","v.投;扔",
"thumb","n./v.拇指",
"thunder","n./v.雷聲;打雷",
"thunderstorm","n.雷雨",
"Thursday","n.星期四",
"thus","adv.這樣",
"thyroid","n.甲狀腺",
"ticket","n.票",
"tide","n.潮汐",
"tidy","adj.整潔的",
"tie","n.領帶",
"tie","v.繫;捆",
"tiger","n.老虎",
"tight","adj./adj.緊的;緊緊地",
"tighten","v.(使)繃緊",
"tightly","adv.堅固地",
"till","prep./conj.直到",
"timber","n.木材",
"time","n./v.時間",
"timid","adj.膽小的",
"tiny","adj.非常小的",
"tip","n./v.小費",
"tire","n.輪胎",
"tire","v.疲勞",
"tired","adj.疲倦的",
"tiresome","adj.令人厭煩的",
"tissue","n.組織",
"title","n.標題",
"toast","n./v.吐司;烤;烘",
"tobacco","n.煙草",
"today","adv./n.今天",
"toe","n.腳趾",
"tofu","n.豆腐",
"together","adv.一起",
"toilet","n.洗手間;廁所",
"token","n.記號",
"tolerate","v.容忍",
"toll","n.通行費",
"tomato","n.蕃茄",
"tomb","n.墳",
"tomorrow","adv./n.明天",
"ton","n.噸",
"tone","n.音調",
"tongue","n.舌(頭)",
"tonight","adv./n.今晚",
"tool","n.工具",
"tooth","n.牙齒",
"toothbrush","n.牙刷",
"toothpaste","n.牙膏",
"top","adj./n.頂端的;頂部",
"topic","n.題目",
"tortoise","n.(陸)龜",
"torture","v.拷打;折磨",
"toss","n./v.扔;擲",
"total","adj./n./v.完全的;總數",
"totally","adj.完全地",
"touch","n./v.觸覺;觸摸",
"touchstone","n.試金石;(檢驗)標準",
"tough","adj.堅韌的",
"tour","n./v.旅遊",
"tournament","n.錦標賽",
"tourism","n.觀光業",
"tourist","n.觀光客",
"tow","v./n.拖",
"toward(s)","prep.向",
"towel","n.毛巾",
"tower","n./v.塔樓;屹立",
"town","n.城鎮",
"toxic","adj.有毒的",
"toy","n./v.玩具;玩弄",
"trace","n./v.蹤跡;追溯",
"track","n./v.軌道;追蹤",
"tractor","n.拖拉機",
"trade","n./v.貿易",
"tradition","n.傳統",
"traditional","adj.傳統的",
"traffic","n.交通",
"tragedy","n.悲劇",
"tragic","adj.悲慘的",
"train","n.火車",
"train","v.訓練",
"tranquil","adj.寧靜的",
"tranquilizer","n.鎮靜劑",
"transcend","v.超越",
"transfer","n./v.調動",
"transformation","n.轉變",
"transit","n.運輸",
"transition","n.過渡(期)",
"translate","v.翻譯",
"translation","n.翻譯",
"translator","n.翻譯者",
"transmission","n.傳播",
"transmit","v.傳播",
"transplant","v./n.移植",
"transport","n.運輸(系統)",
"trauma","n.精神創傷;外傷",
"travel","n./v.旅行",
"traveler","n.旅行者",
"traverse","v.橫越",
"tray","n.盤子",
"treasure","n./v.金銀財寶;珍惜",
"treat","n./v.請客;對待",
"tree","n.樹",
"tremble","n./v.發抖",
"tremendous","adj.極大的",
"trend","n.趨勢",
"trespass","v.擅入",
"trial","n.審判",
"triangle","n.三角形",
"tribe","n.部落",
"tribune","n.護民官",
"trick","n./v.詭計;欺騙",
"tricky","adj.狡猾的",
"trigger","n./v.扳機;引發",
"trim","v.修剪",
"trip","n.旅行",
"triumph","n./v.勝利;戰勝",
"triumphant","adj.勝利的",
"trivial","adj.瑣碎的",
"troop","n.部隊",
"trophy","n.獎品",
"tropical","adj.熱帶的",
"trouble","n./v.憂慮",
"troublesome","adj.令人討厭的",
"trousers","n.褲子",
"truck","n./v.卡車;用卡車裝運",
"true","adj.真實的",
"trumpet","n./v.喇叭;鼓吹",
"trunk","n.樹幹",
"trust","n./v.信任",
"truth","n.事實",
"try","n./v.嘗試",
"tsunami","n.(源自日語)海嘯",
"tub","n 浴缸;盆",
"tube","n.管子",
"tuberculosis","n.肺結核",
"Tuesday","n.星期 v.",
"tuition","n.學費",
"tumble","v.跌倒",
"tumor","n.腫瘤",
"tune","n./v.曲調",
"tunnel","n.隧道",
"turkey","n.火雞",
"turmoil","n.混亂",
"turn","n./v.轉動",
"turnpike","n.付費高速公路",
"turtle","n.(海)龜",
"tutor","n.家庭教師",
"twelve","adj./n.十二",
"twenty","adj./n.二十",
"twice","adv.兩次",
"twig","n.細枝",
"twin","n.孿生子之一",
"twinkle","n./v.閃爍",
"twist","n./v.彎曲;扭曲",
"two","adj./n.二",
"type","n.類型",
"type","v.打字",
"typewriter","n 打字機",
"typhoon","n.颱風",
"typical","adj.典型的",
"typist","n.打字員",
"tyrant","n.暴君",
"ugly","adj.醜陋的",
"ulcer","n.潰瘍",
"ultimately","adv.最後",
"ultraviolet","adj.紫外(線)的",
"umbrella","n.雨傘",
"unanimously","adv.全體一致地",
"unaware","adj.沒有察覺到的",
"uncle","n.伯父",
"uncomfortable","adj.不舒服的",
"unconsciously","adv.無意識地",
"under","prep./adv.在...下面",
"underdeveloped","adj.低度開發的",
"underestimate","v.低估",
"undergo","v.接受;經歷",
"undergraduate","n.大學肄業生",
"underground","adj.地(面)下的;祕密的",
"underline","在...之下劃線",
"understandable","adj.可理解的",
"undertake","v.承擔",
"underwear","n.內衣",
"underwrite","v.為...保險",
"undesirable","adj.令人不快的",
"undue","adj.過份的",
"unemployed","adj.失業的",
"unemployment","n.失業人數",
"unexpectedly","adj.未料到地",
"unfair","adj.不公平的",
"unforgettable","adj.難忘的",
"unfortunate","adj.不幸的",
"ungrateful","adj.忘恩負義的",
"uniform","adj./n.相同的;制服",
"unify","v.統一",
"union","n.結合",
"unit","n.單位",
"unite","v.合併",
"unity","n.統一",
"universe","n.宇宙",
"university","n.大學",
"unless","conj.除非",
"unlike","prep.不像",
"unlimited","adj.無限的",
"unload","v.從...卸下(貨物)",
"unlock","v.開啟",
"unofficial","adj.非正式的",
"unpleasant","adj.使人不愉快的",
"unprecedented","adj.前所未有的",
"unpredictable","adj.不可預測的",
"unrest","n.動亂",
"until","prep./conj.直到...時",
"untouched","adj.未觸動過的",
"unusually","adv.非常",
"up","prep./adv.向上地",
"upbringing","n.養育",
"upend","v.把...倒置",
"upgrade","v.提升",
"uplift","v.抬起",
"upper","adj.較高的",
"uproot","v.根除",
"upset","adj./v.心煩的;使生氣",
"upstairs","adj.在樓上",
"uranium","n.鈾",
"urban","adj.城市的",
"urge","n./v.衝動;催促",
"urgency","n.緊急",
"urgent","adj.緊急的",
"us","pron.我們",
"usage","n.使用方法",
"use","n./v.益處;使用",
"used","adj.舊的",
"useful","adj.有用的",
"usefulness","n.有效",
"useless","adj.無用的",
"usher","n.迎賓員",
"usual","adj.通常的",
"utensil","n.器皿",
"utmost","adj.極度的",
"Utopia","prop./n.烏托邦",
"utterly","adj.完全地",
"vacancy","n.空位",
"vacant","adj.空的",
"vacation","n.假期",
"vaccine","n.疫苗",
"vacuum","n.真空;空虛(感)",
"vaguely","adv.模糊地",
"vain","adj./n.無效的",
"validity","n.有效力",
"valley","n.山谷",
"valuable","adj.珍貴的",
"value","n./v.價值;估價",
"vanilla","n.香草",
"vanish","v.消失",
"vanity","n.自負",
"vapor","n.水蒸氣",
"variable","adj.易變的",
"variation","n.變化",
"variety","n 多樣化",
"various","adj.各式各樣的",
"vary","v.變化",
"vase","n.花瓶",
"vast","adj.廣闊的",
"vegetable","n.蔬菜",
"vegetarian","adj./n.素食者",
"vehicle","n.車輛",
"veil","n.面紗",
"vent","n.通風孔",
"venture","n.冒險(行為);投機",
"Venus","n.維納斯女神;金星",
"verb","n.動詞",
"verbal","adj.口頭的",
"verdict","n.判決",
"verify","v.證實",
"versatile","adj.多才多藝的",
"verse","n.詩節",
"vertical","adj.垂直的",
"very","adv.非常",
"vet","n./v.獸醫;審查",
"vex","v.使惱怒",
"via","prep.取道",
"viable","adj.切實可行的",
"vibration","n.振動",
"vicious","adj.惡意的",
"victim","n.受害者",
"victimize","v.使犧牲",
"victory","n.勝利",
"video","n.錄影帶",
"view","n./v.景色;看",
"vigor/vigour","n.精力",
"vigorously","adv.精力充沛地",
"villa","n.別墅",
"village","n.村莊",
"villain","n.反派角色",
"vine","n.葡萄(藤)",
"vinegar","n.醋",
"vineyard","n.葡萄園",
"violate","v.違背;侵犯",
"violence","n.暴力",
"violent","adj.兇暴的",
"violet","adj./n.紫羅蘭",
"violin","n.小提琴",
"virtually","adv.幾乎",
"virtuous","adj.品德高尚的",
"virus","n.病毒",
"visa","n.簽證",
"visible","adj.看得見的",
"vision","n.視力",
"visit","n./v.探望",
"visitor","n.探訪者",
"visual","adj.視覺的",
"visualize","v.想像",
"vital","adj.維持生命所必需的",
"vitamin","n.維他命",
"vivid","adj 生動的",
"vocabulary","n.詞彙",
"vocal","adj.發音的",
"vocation","n.職業",
"vogue","n.流行",
"voice","n./v.聲音;表達",
"volcano","n.火山",
"volleyball","n.排球",
"volume","n.音量",
"voluntary","adj.自願的",
"volunteer","n./v.志願者;自願",
"vomit","v.嘔吐",
"vote","n./v.投票",
"voter","n.選民",
"vow","n.誓言",
"vowel","n.母音",
"voyage","n.航行",
"vulgar","adj.粗俗的",
"vulnerable","adj.脆弱的",
"wagon","n.四輪運貨馬車",
"waist","n.腰",
"wait","n./v.等候",
"waiter","n.侍者",
"waitress","n.女侍",
"wake","v.醒來",
"waken","v.弄醒",
"walk","n./v.走",
"Walkman","n.隨身聽",
"wall","n.牆",
"wallet","n.皮夾子",
"walnut","n.胡桃(樹)",
"wander","v.徘徊(於)",
"want","n./v. 想要",
"warn","v.警告",
"warning","n.警告",
"warrant","n.搜查令",
"warrior","n.戰士",
"warship","n.軍艦",
"wash","n./v.洗滌",
"waste","n./v.浪費",
"watch","n.手錶",
"watch","v.觀看",
"water","n./v.水",
"waterfall","n.瀑布",
"watermelon","n.西瓜",
"wave","n/v.波;揮動",
"wax","n.蠟",
"way","n.路",
"wayside","n.路邊",
"weak","adj.虛弱的",
"weaken","v.削弱",
"wealth","n.財富",
"wealthy","adj.富裕的",
"weapon","n.武器",
"wear","n./v.穿;戴",
"weary","adj.筋疲力竭的",
"weather","n.天氣",
"weave","v.紡織",
"web","n.網",
"wedding","n.婚禮",
"Wednesday","n.星期三",
"weed","n.雜草",
"week","n.週",
"weekly","adj./adv.一週一次的",
"weekday(s)","n.工作日",
"weekend","n.週末",
"weep","v.哭泣",
"weigh","v.稱...的重量",
"weight","n.重量",
"welcome","n./v.歡迎",
"welfare","n.福利",
"well","adv.很好地",
"well","n.水井",
"West","adj./adv./n.西方的",
"western","adj./n.西方世界",
"wet","adj.溼的",
"whale","n.鯨",
"what","pron./conj.什麼",
"whatever","adj./advJpron./conj. 任何的;無論什麼",
"whatsoever","adj.任何的",
"wheat","n.小麥",
"wheel","n./v.輪子;旋轉",
"wheelchair","n:輪椅",
"when","pron./adv./conj. 那時;什麼時候",
"whenever","adv./conj.無論什麼時候;每當",
"where","adv./pron./conj. 在哪裡;哪裡",
"whereabouts","adv.在哪兒",
"whereby","adv.憑藉",
"wherever","conj./adv.無論到哪裡",
"whether","conj.是否",
"which","pron.哪一個",
"whichever","adj./pron.無論哪個",
"while","conj/n.當...的時候",
"whisper","n./v.低語",
"whistle","n./v.哨子;吹口哨",
"white","adj./n.白色的",
"who","pron.誰",
"whoever","pron.無論是誰",
"whole","adj./n.全部的",
"wholesale","n./adj.批發;大批的",
"wholly","adv.完全地",
"whom","pron.誰",
"whose","pron.誰的",
"why","adv./pron./conj. 為什麼",
"wicked","adj.邪惡的",
"wide","adj.寬的",
"widen","v.(使)變寬",
"widower","n.鰥夫",
"width","n.寬度",
"wife","n.妻子",
"wild","adj.野生的",
"wilderness","n.荒野",
"wildly","adv.野生地",
"will","(1) n.意志力",
"will","(2) v.將",
"willing","adj.願意的",
"willingness","n.自願",
"win","n./v.勝利;獲勝",
"wind","(1)n.風",
"wind","(2) v.搖動",
"window","n.窗",
"windy","adj.多風的",
"wine","n./v.葡萄酒",
"wing","n./v.翅膀;飛行",
"winner","n.獲勝者",
"winter","n.冬天",
"wipe","v.擦",
"wire","n./v.金屬線",
"wisdom","n.智慧",
"wise","adj.聰明的",
"wish","n./v.希望",
"witch","n.女巫",
"with","prep.與",
"withhold","v.保留",
"within","prep./adv.在...之內",
"without","prep./adv.沒有",
"withstand","v.抵擋",
"witness","n./v.目擊者",
"woe","n.悲傷",
"wolf","n.狼",
"wolf","n.色狼",
"woman","n.女人",
"wonder","n./v.想要知道",
"wonderful","adj.極好的",
"wood(s)","n.樹林",
"wool","n.羊毛",
"word","n.字",
"work","n./v.工作",
"worker","n.工人",
"world","n.世界",
"worm","n.蟲",
"worried","adj.煩惱的",
"worry","n./v.煩惱",
"worse","adj./adv.更壞的",
"worship","v.崇拜",
"worth","adj./n.價值",
"worthy","adj.值得...的",
"wound","n./v.傷口;使受傷",
"wrap","n./v.圍巾;包裹",
"wreckage","n.殘骸",
"wrist","n.腕",
"write","v.寫",
"writer","n.作家",
"writing","n.作品",
"wrong","adj./adv./n./v. 錯誤的 Y",
"yacht","n.帆船;遊艇",
"Yale","n.耶魯大學",
"yard","n.碼",
"yawn","n./v.打呵欠",
"yeah","adv.對",
"year","n.年",
"yearly","adj./adv.每年一次",
"yearn","v.渴望",
"yeast","n.酵母(菌)",
"yell","v.喊叫",
"yellow","adj./n.黃色的",
"yesterday","adv./n.昨天",
"yet","conj./adv.但是;還",
"yield","v.出產",
"youngster","n.年輕人",
"youth","n.青春",
"youthful","adj.年輕的 Z",
"zeal","n 渴望",
"zealously","adv.熱心地",
"zebra","n.斑馬",
"zero","n.零",
"zip","n.活力",
"zone","n.地帶",
"zoo","n.動物園",
"zoology","n.動物學"
            };
    }
}
